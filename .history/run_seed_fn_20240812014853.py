...
def run_seed(
    ...
    if cfg.method.name == "ARM":
        ...
        for replay_file in replay_files:
            ...
            # 直接对比两个文件的内容
            if not filecmp.cmp(replay_file, output_file_path_before, shallow=False):
                print(f"Warning: Files {replay_file} and {output_file_path_before} differ!")
            else:
                print(f"Files {replay_file} and {output_file_path_before} are identical.")
