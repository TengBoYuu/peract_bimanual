...
def create_bimanual_replay(
    ...
    for cname in cameras:
        ...
        observation_elements.append(
            ObservationElement("%s_depth" % cname, (1, image_size[1], image_size[0]), np.float16)
        )  # see pyrep/objects/vision_sensor.py on how pointclouds are extracted from depth frames
        observation_elements.append(
            ObservationElement(
    ...
    replay_buffer = TaskUniformReplayBuffer(
        ...
        timesteps=timesteps, # 历史信息
        replay_capacity=int(replay_size),
        action_shape=(8 * 2,),
        action_dtype=np.float32,
        reward_shape=(),
        reward_dtype=np.float32,
        update_horizon=1, # 预测未来多少时间步
...
def create_unimanual_replay(
    ...
    for cname in cameras:
        ...
        observation_elements.append(
            ObservationElement("%s_depth" % cname, (1, *image_size), np.float16)
        )  # see pyrep/objects/vision_sensor.py on how pointclouds are extracted from depth frames
        observation_elements.append(
            ObservationElement(
                "%s_camera_extrinsics" % cname,
...
def deep_compare(obj1, obj2):
    # 如果两个对象是 numpy 数组，使用 np.array_equal 比较
    if isinstance(obj1, np.ndarray) and isinstance(obj2, np.ndarray):
        return np.array_equal(obj1, obj2)
    
    # 如果对象是 BimanualObservation，逐个属性进行比较
    elif isinstance(obj1, BimanualObservation) and isinstance(obj2, BimanualObservation):
        return (
            deep_compare(obj1.perception_data, obj2.perception_data) and
            deep_compare(obj1.task_low_dim_state, obj2.task_low_dim_state) and
            deep_compare(obj1.misc, obj2.misc) and
            deep_compare(obj1.right, obj2.right) and
            deep_compare(obj1.left, obj2.left)
        )
    
    # 如果对象是 UnimanualObservationData，逐个属性进行比较
    elif isinstance(obj1, UnimanualObservationData) and isinstance(obj2, UnimanualObservationData):
        return (
            deep_compare(obj1.joint_velocities, obj2.joint_velocities) and
            deep_compare(obj1.joint_positions, obj2.joint_positions) and
            deep_compare(obj1.joint_forces, obj2.joint_forces) and
            deep_compare(obj1.gripper_open, obj2.gripper_open) and
            deep_compare(obj1.gripper_pose, obj2.gripper_pose) and
            deep_compare(obj1.gripper_matrix, obj2.gripper_matrix) and
            deep_compare(obj1.gripper_joint_positions, obj2.gripper_joint_positions) and
            deep_compare(obj1.gripper_touch_forces, obj2.gripper_touch_forces) and
            deep_compare(obj1.ignore_collisions, obj2.ignore_collisions)
        )
    
    # 如果对象是字典，逐个键值对进行递归比较
    elif isinstance(obj1, dict) and isinstance(obj2, dict):
        return len(obj1) == len(obj2) and all(
            k in obj2 and deep_compare(obj1[k], obj2[k]) for k in obj1
        )
    
    # 如果对象是列表或元组，逐个元素递归比较
    elif isinstance(obj1, (list, tuple)) and isinstance(obj2, (list, tuple)):
        return len(obj1) == len(obj2) and all(deep_compare(o1, o2) for o1, o2 in zip(obj1, obj2))
...
def fill_replay(
    ...
    for d_idx in range(num_demos):
        ...
        # print(obs_config)
        demo1 = rlbench_utils.get_stored_demos(
            amount=1,
            image_paths=False,
            dataset_root=cfg.rlbench.keypoint_path,
            variation_number=-1,
            task_name=task,
            obs_config=obs_config,
            random_selection=False,
        ...
        print("successfully created demo1")
        ...
        # print(len(demo))
        # print(len(demo1))
        # # print(demo[0]==demo1[0])
        # # 使用比较函数
        # print(type(demo[0]))
        # print("Demo objects are equal:", deep_compare(demo[1], demo1[0]))

        indices_in_demo = []
        # 遍历 demo1 中的每个元素
        for item in demo1:
            # 遍历 demo，找到与当前 item 匹配的索引
            for idx, demo_item in enumerate(demo):
                if deep_compare(item, demo_item):
                    indices_in_demo.append(idx)
        print(indices_in_demo)
        episode_keypoints = indices_in_demo
        if rank == 0:
            logging.info(
