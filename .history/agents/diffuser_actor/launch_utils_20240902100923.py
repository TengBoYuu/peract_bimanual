...
from agents.diffuser_actor.diffuser_actor import DiffuserActor
...
def create_agent(cfg: DictConfig):
    ...
    qattention_agents = []
    for depth, vox_size in enumerate(cfg.method.voxel_sizes): # 循环只执行一次
        ...
        # )
        diffuser_actor = DiffuserActor(
            gripper_loc_bounds=cfg.method.bounds_offset[depth - 1] if depth > 0 else None,
        )
        qattention_agent = QAttentionDiffuserActorAgent(
            ...
            option_selector_args=cfg.method.option_selector,
        )
