...
from agents.diffuser_actor.utils.layers import (
    FFWRelativeSelfAttentionModule,
    FFWRelativeCrossAttentionModule,
    FFWRelativeSelfCrossAttentionModule
)
from agents.diffuser_actor.utils.encoder import Encoder
from agents.diffuser_actor.utils.layers import ParallelAttention
from agents.diffuser_actor.utils.position_encodings import (
    RotaryPositionEncoding3D,
    SinusoidalPosEmb
)
from agents.diffuser_actor.utils.utils import (
...
class DiffuserActor(nn.Module):
    ...
    def compute_trajectory(
        ...
        fixed_inputs = self.encode_inputs(
            rgb_obs, pcd_obs, instruction
        )

        # Condition on start-end pose
        B, L, D = fixed_inputs[0].shape[0], trajectory_mask.size(1), 9
        ...
        # Back to quaternion => rotation
        trajectory = self.unconvert_rot(trajectory)
        # unnormalize position => trans
        trajectory[:, :, :3] = self.unnormalize_pos(trajectory[:, :, :3])
        # Convert gripper status to probability => open
        if trajectory.shape[-1] > 7:
            trajectory[..., 7] = trajectory[..., 7].sigmoid()
        # Convert gripper status to probability => collision
        if trajectory.shape[-1] > 8:
            trajectory[..., 8] = trajectory[..., 8].sigmoid()

        return trajectory
    def normalize_pos(self, pos):
        pos_min = pos[..., :3].min(dim=-1, keepdim=True)[0]
        pos_max = pos[..., :3].max(dim=-1, keepdim=True)[0]
        diff = pos_max - pos_min
        if torch.any(pos_max == pos_min):
            equal_elements = (pos_max == pos_min)
            diff[equal_elements] = 1e-5
            # print("Warning: pos_max and pos_min are the same at some positions, causing division by zero.")
            # print("Equal positions:", torch.nonzero(equal_elements))
            # print("pos_max at these positions:", pos_max[equal_elements])
            # print("pos_min at these positions:", pos_min[equal_elements])
            # print(pos_max)
            # print(pos_min)
        return (pos - pos_min) / (diff) * 2.0 - 1.0
    ...
    def unnormalize_pos(self, pos):
        ...
        if torch.any(pos_max == pos_min):
            ...
            # print(pos_max)
            # print(pos_min)
        return (pos + 1.0) / 2.0 * (diff) + pos_min
    ...
    def forward(
        ...
        """
        # print("--------diffuser actor----------")
        # print(gt_trajectory.shape) # [2,1,9]
        # print(trajectory_mask.shape) # [2,1]
        # print(rgb_obs.shape) # [2,6,3,256,256]
        ...
        # print("pos - pos min", pos-pos_min) # max = min ???
        # print("max - min", pos_max - pos_min)
        # print((pos - pos_min) / (pos_max - pos_min) * 2.0 - 1.0)
        # print(((pos - pos_min) / (pos_max - pos_min) * 2.0 - 1.0).shape)
        #######
        ...
        for layer_pred in pred:
            trans = layer_pred[..., :3]
            rot = layer_pred[..., 3:9]
            trans_loss += 30 * F.l1_loss(trans, noise[..., :3], reduction='mean')
            rot_loss += 10 * F.l1_loss(rot, noise[..., 3:9], reduction='mean')


            if torch.numel(gt_openess) > 0:
                openess = layer_pred[..., 9:10]
                grip_loss += F.binary_cross_entropy_with_logits(openess, gt_openess)
            if torch.numel(gt_collision) > 0:
                collision = layer_pred[..., 10:]
                collision_loss += F.binary_cross_entropy_with_logits(collision, gt_collision)
        return trans_loss, rot_loss, grip_loss, collision_loss
...
class DiffusionHead(nn.Module):
    ...
    def forward(self, trajectory, timestep,
        ...
        pos_pred, rot_pred, openess_pred, collision_pred = self.prediction_head(
            trajectory[..., :3], traj_feats,
            context[..., :3], context_feats,
            timestep, fps_feats, fps_pos,
            instr_feats
        )
        return [torch.cat((pos_pred, rot_pred, openess_pred, collision_pred), -1)]

    def prediction_head(self,
                        gripper_pcd, gripper_features,
                        context_pcd, context_features,
                        timesteps, sampled_context_features, sampled_rel_context_pos,
                        instr_feats):
        """
        Compute the predicted action (position, rotation, opening, collision).
        ...
        # collision head from position head
        collision = self.collision_predictor(position_features)
        return position, rotation, openess, collision
