...
class DiffuserActor(nn.Module):
    ...
    def forward(
        ...
        if gt_trajectory is not None:
            ...
            gt_openess = gt_trajectory_copy[...,7]
            gt_collision = gt_trajectory_copy[...,-1]
        print("gt_openess: ",gt_openess)
        ...
        for layer_pred in pred:
            ...
            print("torch.numel(gt_openess): ",torch.numel(gt_openess))
            print("torch.numel(gt_collision): ",torch.numel(gt_collision))
            if torch.numel(gt_openess) > 0:
                openess = layer_pred[..., 9:10]
                print("openess: ", openess.shape)
                print("gt_openess: ", gt_openess.shape)
                grip_loss += F.binary_cross_entropy_with_logits(openess, gt_openess)
            if torch.numel(gt_collision) > 0:
                collision = layer_pred[..., 10:]
                print("collsion: ",collision.shape) # [B,1,1]
                print("gt_collision: ", gt_collision.shape) # [B,1]
