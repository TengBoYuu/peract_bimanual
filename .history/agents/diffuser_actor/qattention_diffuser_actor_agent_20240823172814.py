...
class QAttentionDiffuserActorAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        if self._training:
            ...
            # 假设你有一个模型 self._q，并且要将符合条件的参数名称写入 'params_names.txt'
            # 假设你有一个模型 self._q，并且要将符合条件的参数名称及其参数量写入 'params_info.txt'
            with open('/mnt/disk_1/tengbo/peract_bimanual/params_info.txt', 'w') as f:
                for name, p in self._q.named_parameters():
                    if p.requires_grad:
                        param_count = p.numel()  # 获取参数的数量
                        f.write(f"{name}: {param_count}\n")  # 写入文件，格式为 '参数名称: 参数量'
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # collision = action_ignore_collisions.unsqueeze(0).expand(gt_.size(0),1)
        ...
        # print('======> translation shape', q_trans.shape) # [B,1,100,100,100]
        # print('======> rot_and_grip shape',q_rot_grip.shape) # [B,218]
        # print('======> collision shape',q_collision.shape) # [B,2]
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        coords = out[0,:3]
        rot_grip_action = out[0,3:7]
        ignore_collisions_action = out[0,-1]
