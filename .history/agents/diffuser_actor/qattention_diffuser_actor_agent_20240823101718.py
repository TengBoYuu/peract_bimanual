...
class QAttentionDiffuserActorAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        rgb = [r.unsqueeze(1) for r in rgb]
        rgb_obs = torch.cat(rgb, dim=1) # [B,6,3,256,256]
        pcd = [p.unsqueeze(1) for p in pcd]
        pcd_obs = torch.cat(pcd, dim=1) # [B,6,3,256,256]
        ...
        pos_l2 = ((out[..., :3] - gt[..., :3]) ** 2).sum(-1).sqrt()
        # symmetric quaternion eval
        quat_l1 = (out[..., 3:7] - gt[..., 3:7]).abs().sum(-1)
        quat_l1_ = (out[..., 3:7] + gt[..., 3:7]).abs().sum(-1)
        select_mask = (quat_l1 < quat_l1_).float()
        quat_l1 = (select_mask * quat_l1 + (1 - select_mask) * quat_l1_)
        # gripper openess
        openess = ((out[..., 7:8] >= 0.5) == (gt[..., 7:8] > 0.0)).bool()
        collisioness = ((out[..., 8:] >= 0.5) == (gt[...,8:] > 0.0)).bool()
        q_trans_loss = pos_l2.mean()
        q_rot_loss = quat_l1.mean()
        q_grip_loss = openess.flatten().float().mean()
        q_collision_loss = collisioness.flatten().float().mean()
        combined_losses = (
