...
class DiffuserActor(nn.Module):
    ...
    def encode_inputs(self, visible_rgb, visible_pcd, instruction):
        ...
        for i, tensor in enumerate(rgb_feats_pyramid):
            if torch.isnan(rgb_feats_pyramid).any():
                print("rgb nan")
        for i, tensor in enumerate(pcd_pyramid):
            if torch.isnan(pcd_pyramid).any():
                print("pcd nan")
    ...
    def forward(
        ...
        # print("gt_trajectory: ",gt_trajectory.shape) # [2,1,9]
        ...
        if torch.isnan(noisy_trajectory).any(): # nan exist
            print("noisy_tra nan")
        if torch.isnan(timesteps).any():
            print("time nan")
        for i, tensor in enumerate(fixed_inputs): # nan exist
...
class DiffusionHead(nn.Module):
    ...
    def prediction_head(self,
        ...
        # print("position: ", position_features)
    ...
    def predict_pos(self, features, rel_pos, time_embs, num_gripper,
        ...
        # print("time_embs: ", time_embs)
        # print("instr_feats: ", instr_feats)
