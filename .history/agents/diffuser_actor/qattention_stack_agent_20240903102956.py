...
class QAttentionStackAgent(Agent):
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        # print("-------rot_grip_results------",rot_grip_results[0].shape)
        # print(ignore_collisions_results[0].shape)
        # print(translation_results[0].shape)
        ...
        action_combined = torch.cat([tensor.cpu() for tensor in act_results.action], dim=-1).flatten()
        continuous_action = action_combined.numpy()
        # print("-----continuous-------")
        # print(continuous_action) # 这里就是最终的连续值了
        return ActResult(
