...
class QFunction(nn.Module):
    def __init__(
        ...
        self._qnet = diffuser_actor.to(device)
    ...
    def forward(
        ...
        # voxel_grid (b,10,100,100,100)
        # lang_goal_emb (b,1024)
        # lang_token_embs (b,77,512)
        # 用以上2个去做option
        states = voxel_grid.reshape(b,10,-1)
        ...
        if inference:
            ...
            return out, voxel_grid
        else:
            trans_loss, rot_loss, grip_loss, collision_loss = self._qnet(
                gt_trajectory,
                trajectory_mask,
                rgb_obs,
                pcd_obs,
            ...
            return trans_loss, rot_loss, grip_loss, collision_loss, voxel_grid
...
class QAttentionDiffuserActorAgent(Agent):
    def __init__(
        ...
        self._diffuser_actor = diffuser_actor
        ...
        self.option_args = option_selector_args
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # print("------------------------",prev_layer_bounds) # None
        proprio = None
        if self._include_low_dim_state:
            proprio = replay_sample["low_dim_state"]

        # print(proprio.shape) # [B,4]
        obs,rgb, pcd = self._preprocess_inputs(replay_sample)
        ...
        # action是ground truth
        # 这里 action_trans action_rot_grip拼接后[B,7]，满足diffusion_actor输入，但是差一个length维度，只是这一帧的
        # print('======> action_trans shape', action_trans.shape) # [B,3]
        # print('======> action_rot_grip shape', action_rot_grip.shape) # [B,4]
        # print('======> pcd shape', pcd.shape) # list
        # print(len(pcd)) # 6
        # print(pcd[0].shape) # [B,3,256,256]
        # forward pass 
        # print(len(rgb)) # 6
        # print(rgb[0].shape) # [B,3,256,256]
        # print(action_gripper_pose.shape) # [B,7]
        # 只用gt_action和obs就好
        
        # 需要使用的数据：rgb pcd action_gripper_pose(gt) lang_token_embs
        # action_gripper_pose既当gt又当curr???

        # diffuser 返回的就是total loss？
        rgb_ = [r.unsqueeze(1) for r in rgb]
        rgb_obs = torch.cat(rgb_, dim=1) # [B,6,3,256,256]
        pcd_ = [p.unsqueeze(1) for p in pcd]
        pcd_obs = torch.cat(pcd_, dim=1) # [B,6,3,256,256]
        instructions = lang_token_embs # [B,77,512]
        ...
        open = proprio[0][0]
        open_expanded = open.unsqueeze(0).expand(action_gripper_trans.size(0), 1)
        gt = torch.cat((action_gripper_trans,action_gripper_quat_wxyz,open_expanded, action_ignore_collisions), dim=1) 
        gt = gt.unsqueeze(1) 
        # indicies action
        # action_trans = action_trans * 0.01
        # rot_grip = action_rot_grip[..., :3] * self._rotation_resolution
        # quaternion = self.euler_to_quaternion(rot_grip) # wxyz
        # openess = action_rot_grip[...,-1].unsqueeze(-1)
        # gt_action = torch.cat((action_trans,quaternion,openess,action_ignore_collisions), dim=1)
        ...
        q_trans_loss, q_rot_loss, q_grip_loss, q_collision_loss, voxel_grid = self._q(
            ...
            prev_layer_voxel_grid,
            gt_trajectory=gt,
            trajectory_mask=torch.ones((bs, 1), dtype=torch.bool),
            rgb_obs=rgb_obs,
            pcd_obs=pcd_obs,
            instruction=instructions,
            inference=False,
        )
        if torch.isnan(q_trans_loss).any():
            raise ValueError(f"q_trans_loss has NaN values: {q_trans_loss}")
        if torch.isnan(q_rot_loss).any():
            raise ValueError(f"q_rot_loss has NaN values: {q_rot_loss}")
        if torch.isnan(q_grip_loss).any():
            raise ValueError(f"q_grip_loss has NaN values: {q_grip_loss}")
        if torch.isnan(q_collision_loss).any():
            raise ValueError(f"q_collision_loss has NaN values: {q_collision_loss}")
        combined_losses = (
        ...
        if step % 10 == 0 and rank==0:
            ...
            wandb.log({
                'train/grip_loss': q_grip_loss,
                'train/trans_loss': q_trans_loss,
                'train/rot_loss': q_rot_loss,
                'train/collision_loss': q_collision_loss,
        ...
        self._summaries = {
            ...
            "losses/trans_loss": q_trans_loss,
            "losses/rot_loss": q_rot_loss ,
            "losses/grip_loss": q_grip_loss,
            "losses/collision_loss": q_collision_loss
        }
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        obs, rgb,pcd = self._act_preprocess_inputs(observation)
        bs = pcd[0].shape[0]
        rgb_ = [r.unsqueeze(1) for r in rgb]
        rgb_obs = torch.cat(rgb_, dim=1) # [B,6,3,256,256]
        pcd_ = [p.unsqueeze(1) for p in pcd]
        ...
        instructions = lang_token_embs
        rgb_obs = rgb_obs.squeeze(dim=2)
        pcd_obs = pcd_obs.squeeze(dim=2)
        # inference
        out, vox_grid = self._q(
            obs,
            proprio,
            pcd,
            lang_goal_emb,
        ...
        # print("------out-------",out.shape) # [1,9]
        # coords = out[:,:3]
        # rot_grip_action = out[:,3:7]
        # ignore_collisions_action = out[:,-1]
        # ignore_collisions_action = ignore_collisions_action.unsqueeze(1)
        open = torch.where(out[:, 7:8] > 0, torch.tensor(1.0), torch.tensor(0.0))
        coords = torch.abs((out[:,:3] * 100).int())
        euler = torch.abs(((self.quaternion_to_euler(out[:,3:7])/5)).int())
        rot_grip_action = torch.cat((euler,open),dim=1)
        ignore_collisions_action = ((out[:,-1]).int()).unsqueeze(1)
        # print(coords)
        # print(rot_grip_action)
        # rot_grip_action = torch.abs((rot_grip_action*100).int())
        ...
        # print("ignore",ignore_collisions_action.shape)
