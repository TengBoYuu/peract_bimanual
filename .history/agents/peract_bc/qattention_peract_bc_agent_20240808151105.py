...
class QFunction(nn.Module):
    ...
    def forward(
        ...
        # print('-------------------------------lang.shape,',lang_goal_emb.shape,'-------------------------------')
        ...
        flattened_voxel = voxel_grid.view(b,-1)
        states = torch.cat((flattened_voxel, proprio),dim=1)
        options = None
        lang_dim = lang_goal_emb.shape[1]
        timesteps = proprio[-1]
        state_dim = states.shape[1]
        self.option_selector.lang_dim = lang_dim
        self.option_selector.state_dim = state_dim
        # proprio (2,4)
        ...
        selected_options, _, commitment_loss, entropy, state_embeddings = self.option_selector(
            lang_goal_emb, states, timesteps, attention_mask)
