...
class QFunction(nn.Module):
    def __init__(
        ...
        if self.skill_predictor: # 使用该函数后要将 batch_size 设置为1，用多卡跑
    ...
    def forward(
        ...
        # proprio (2,4)
        # voxel_grid (2,10,100,100,100)
        # lang_goal_emb (2,1024)
        # 用以上3个去做option
        states = voxel_grid.reshape(2,10,-1)
        ...
        if self.option_args.skill_predictor:
            ...
            selected_options, _, commitment_loss, entropy, state_embeddings = self.option_selector(
                lang_goal_emb, states)
            print('----------------------options--------------------',selected_options.shape)
        # forward pass
...
class QAttentionPerActBCAgent(Agent):
    def __init__(
        ...
        self.skill_predictor = self.option_args.skill_predictor
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # Q_Function返回的变量
        if self.option_args.skill_predictor:
            q_trans, q_rot_grip, q_collision, voxel_grid, selected_options = self._q(
                obs,
                proprio,
                pcd,
                lang_goal_emb,
