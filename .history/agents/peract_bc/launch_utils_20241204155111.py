...
def create_agent(cfg: DictConfig):
    ...
    qattention_agents = []
    current_dir = os.path.dirname(os.path.abspath(__file__))  # 获取launch_utils.py所在的绝对路径
    pkl_path = os.path.join(current_dir, "../../lang_token.pkl")  # 相对当前文件所在目录返回上两级

    pkl_path = os.path.abspath(pkl_path)
    with open("../../lang_token.pkl", "rb") as f:
