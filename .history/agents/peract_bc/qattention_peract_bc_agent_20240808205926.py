...
class QFunction(nn.Module):
    ...
    def forward(
        ...
        if self.option_args.skill_predictor:
            ...
            # proprio[-1]是时间步
            # 用以上4个去做option
            flattened_voxel = voxel_grid.reshape(b,-1)
            states = torch.cat((flattened_voxel, proprio),dim=1)
            selected_options = None
            lang_dim = lang_goal_emb.shape[1]
            state_dim = states.shape[1]
            timesteps = proprio[-1]
            selected_options, _, commitment_loss, entropy, state_embeddings = self.option_selector(
                lang_goal_emb, states, timesteps)
            return q_trans, q_rot_and_grip, q_ignore_collisions, voxel_grid, selected_options
