...
class QAttentionPerActBCAgent(Agent):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        if self._training:
            ...
            with open('/mnt/disk_1/tengbo/peract_bimanual/params_info.txt', 'w') as f:  # 确保文件是以 'w' 模式打开
                for name, p in self._q.named_parameters():
                    if p.requires_grad and "clip" not in name:
                        param_count = p.numel()
                        f.write(f"{name}: {param_count}\n")  # 将参数名称及其元素数量写入文件

            print("Finished writing to params_info.txt")  # 确认写入操作完成
        else:
...
# eval
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        # print(coords.shape)
        # print(rot_grip_action.shape)
        # print(ignore_collisions_action.shape)
        return ActResult(
