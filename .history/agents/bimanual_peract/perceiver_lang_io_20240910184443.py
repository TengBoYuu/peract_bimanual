...
class PerceiverVoxelLangEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.lang_fusion_type == "seq":
            l_right = self.lang_preprocess(right_lang_token_embs)  # [B,77,1024] -> [B,77,128]
            ins = torch.cat((l_right, ins), dim=1)  # [B,8077,128]
            l_left = self.lang_preprocess(left_lang_token_embs)  # [B,77,1024] -> [B,77,128]
            ins = torch.cat((l_left, ins), dim=1)  # [B,8077,128]
