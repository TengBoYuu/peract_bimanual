...
rgb_swapped = []
for img in rgb:
    # img 是形状为 (1, 3, 256, 256) 的张量，先移除批次维度
    img = img.squeeze(0)  # 现在形状为 (3, 256, 256)
    
    # 交换通道，通道从 [R, G, B] (0, 1, 2) 变为 [B, G, R] (2, 1, 0)
    img_swapped = img[[2, 1, 0], :, :]  # 通道索引交换
    
    # 重新加上批次维度
    img_swapped = img_swapped.unsqueeze(0)  # 现在形状为 (1, 3, 256, 256)
...
class QAttentionPerActBCAgent(Agent):
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        rgb_src = obs[0][0][0].squeeze(0).permute(1, 2, 0)  / 2 + 0.5
        rgb_src = rgb_src.cpu().numpy()
        folder_path = "/mnt/disk_1/tengbo/peract_bimanual/eval_rgb"
        os.makedirs(folder_path, exist_ok=True)
        file_list = os.listdir(folder_path)
        file_list = [f for f in file_list if f.endswith('.png')]
        file_count = len(file_list)
        filename = f"rgb_{file_count}.png"
        file_path = os.path.join(folder_path, filename)
        cv2.imwrite(file_path, rgb_src)
        ...
        folder_path = "/mnt/disk_1/tengbo/peract_bimanual/eval_voxel1"
        os.makedirs(folder_path, exist_ok=True)
        file_list = os.listdir(folder_path)
        file_list = [f for f in file_list if f.endswith('.png')]
        file_count = len(file_list)
