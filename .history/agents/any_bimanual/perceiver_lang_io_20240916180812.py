...
class PerceiverVoxelLangEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.use_prefix or self.use_skill:
            ins_ = self.voxel_split(ins)
            ins_right_, ins_left_ = ins_.chunk(2, dim=1)
            ins_right_1 = ins_right_
            ins_left_1 = ins_left_
        ...
        if self.low_dim_size > 0:
            if self.use_skill:
                ...
                skill_ins_right = torch.cat([ins_right_, p_right], dim=1)  # [B,128,20,20,20]
                p_left = self.proprio_preprocess_left(proprio[:,4:])  # [B,8] -> [B,64]
                p_left = p_left.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, d, h, w)
                skill_ins_left = torch.cat([ins_left_, p_left], dim=1)  # [B,128,20,20,20]
