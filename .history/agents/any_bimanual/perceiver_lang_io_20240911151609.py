...
class PerceiverVoxelLangEncoder(nn.Module):
    ...
    def option_selector(self, rgb_list, lang_input, proprio, device):
        rgb_features_list = []
        ...
        for i in range(predicted_class.size(0)): 
            ...
            mean_embedding = torch.tensor(self.embeddings_dict[vocab_key]).to(device)
    ...
    def forward(
        ...
        if self.predictor:
            ...
            if self.lang_fusion_type == "seq":
                print("seqqqqqqqq")
                device_ = lang_token_embs.device
                right_skill = self.option_selector(rgbs[:4], lang_token_embs, proprio[:,:4], device_)
                right_lang_token_embs = torch.cat((lang_token_embs,right_skill),dim=1)
                l_right = self.lang_preprocess(right_lang_token_embs)  # [B,77,512] -> [B,77,128]
                print(l_right.shape)
                ins_right = torch.cat((l_right, ins), dim=1)  # [B,8077,128]
                print(ins_right.shape)
                left_skill = self.option_selector(rgbs[-4:], lang_token_embs, proprio[:,4:], device_)
                left_lang_token_embs = torch.cat((lang_token_embs,left_skill),dim=1)
                l_left = self.lang_preprocess(left_lang_token_embs)  # [B,77,512] -> [B,77,128]
