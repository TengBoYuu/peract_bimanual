    ...
    def forward(
        ...
        if self.predictor:
            ...
            if self.lang_fusion_type == "seq":
                ...
                # l_right = torch.cat((self.lang_preprocess_right(right_skill),self.lang_preprocess_right(lang_token_embs)),dim=1)  # [B,77,512] -> [B,77,128]
                l_right = self.lang_preprocess_right(right_skill)
                # print(l_right.requires_grad)
                ins_right = torch.cat((l_right, ins), dim=1)  # [B,8077,128]
                # print(ins_right.shape)
                # l_left = torch.cat((self.lang_preprocess_left(left_skill),self.lang_preprocess_left(lang_token_embs)),dim=1)  # [B,77,512] -> [B,77,128]
                l_left = self.lang_preprocess_right(right_skill)
        ...
        if self.predictor:
            print("skill1")
            x_r = repeat(self.latents, "n d -> b n d", b=b)
        ...
        for it in range(self.iterations):
            ...
            if self.predictor:
                ...
                # print(x_right.shape) # [b,2048,512]
