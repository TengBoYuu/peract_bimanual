...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        if self.use_skill:
            lang_feat_dim, lang_emb_dim, lang_max_seq_len = 1024, 512, 78
        else:
        ...
        if self.use_prefix:
            self.voxel_split = Conv3DBlock(
            in_channels=self.input_preprocess.out_channels, 
            out_channels=self.input_preprocess.out_channels * 2, 
            kernel_sizes=1, 
            strides=1,
            norm=None,
            activation=activation, 
        ...
        if self.use_skill:
            if self.lang_fusion_type == "concat":
                self.lang_preprocess_right = nn.Linear(lang_feat_dim, self.im_channels)
                self.lang_preprocess_left = nn.Linear(lang_feat_dim, self.im_channels)
            elif self.lang_fusion_type == "seq":
                self.lang_preprocess = nn.Linear(lang_emb_dim, self.im_channels * 2)
                self.lang_preprocess_right = nn.Linear(lang_emb_dim, self.im_channels * 2)
                self.lang_preprocess_left = nn.Linear(lang_emb_dim, self.im_channels * 2)
        else:
            if self.lang_fusion_type == "concat":
    ...
    def forward(
        ...
        ins_right_ = ins_left_ = ins # use_prefix = False, ins_right = ins_left
        if self.use_prefix:
            ins_ = self.voxel_split(ins)
            ins_right_, ins_left_ = ins_.chunk(2, dim=1)

        if self.use_skill:
            ins_1 = self.voxel_split1(ins)
            ins_right_1, ins_left_1 = ins_1.chunk(2, dim=1)
        ...
        if self.low_dim_size > 0:
            ...
            ins_right_ = torch.cat([ins_right_, p], dim=1)  # [B,128,20,20,20]
            ins_left_ = torch.cat([ins_left_, p],dim=1)
        ...
        ins_right_ = rearrange(ins_right_, "b d ... -> b ... d")
        ins_left_ = rearrange(ins_left_, "b d ... -> b ... d")
        if self.use_skill:
        ...
        ins_right_ = rearrange(ins_right_, "b ... d -> b (...) d")  # [B,8000,128]
        ins_left_ = rearrange(ins_left_, "b ... d -> b (...) d")  # [B,8000,128]
        if self.use_skill:
            skill_ins_right = rearrange(skill_ins_right, "b ... d -> b (...) d")  # [B,8000,128]
            skill_ins_left = rearrange(skill_ins_left, "b ... d -> b (...) d")  # [B,8000,128]
            l2_distance_scalar = torch.norm(skill_ins_right - skill_ins_left, p=2).item()
        ...
        if self.use_skill:
            if self.lang_fusion_type == "seq":
                ...
                right = self.lang_preprocess_right(right_skill) # [B,77,512] -> [B,77,128]
                # print("l_right" ,l_right.shape)
                l_right = torch.cat((right, l), dim=1)
                ins_right = torch.cat((l_right, ins_right_), dim=1)  # [B,8077,128]
                # print(ins_right.shape)
                left_skill = self.option_selector(skill_ins_left,l)
                left_lang_token_embs = torch.cat((left_skill,lang_token_embs),dim=1)
                l_left = self.lang_preprocess_left(left_lang_token_embs) # [B,77,512] -> [B,77,128]
                ins_left = torch.cat((l_left, ins_left_), dim=1)  # [B,8077,128]
