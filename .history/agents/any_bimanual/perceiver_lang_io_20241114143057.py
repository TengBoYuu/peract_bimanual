...
def vis_voxel(voxel):
    reshaped_data = voxel
    reshaped_data = voxel.reshape(-1, 128)
    
    # 如果数据在 GPU 上，移动到 CPU，并转换为 NumPy 数组
    ...
    # 添加 Alpha 通道，形状变为 (8000, 4)
    alpha_channel = np.ones((colors.shape[0], 1))
    colors = np.concatenate((colors, alpha_channel), axis=1)
    
    # 将颜色数组转换为颜色元组列表
    colors_tuple = [tuple(c) for c in colors]
    
    # 创建体素数据和 facecolors 数组，形状为 (20, 20, 20)
    voxel_data = np.ones((20, 20, 20), dtype=bool)
    facecolors = np.empty(voxel_data.shape, dtype=object)
    facecolors.ravel()[:] = colors_tuple  # 直接赋值给展平的数组
    
    # 创建绘图
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # 绘制体素，使用颜色元组数组作为 facecolors 参数
    ax.voxels(voxel_data, facecolors=facecolors, edgecolor='k')
