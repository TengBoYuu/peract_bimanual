...
class OptionSelector(nn.Module):
    ...
    def init_fc2_weights(self):
        # 初始化 fc2 的权重为 embeddings_matrix，偏置设置为 0
        with torch.no_grad():  # 禁用梯度计算来避免影响模型训练
            self.fc2.weight.copy_(self.embeddings_matrix)  # 将权重初始化为 embeddings_matrix
            self.fc2.bias.zero_()  # 将偏置初始化为 0
    ...
    # 确保 lang_input 可以计算梯度
        ...
        selected_embedding_flat = torch.matmul(one_hot_predicted_class, self.embeddings_matrix.to(self.device))  # [1, 77*512]
