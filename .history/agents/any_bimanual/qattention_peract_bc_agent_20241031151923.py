...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        print(right_q_trans[0].shape)
        rendered_img = visualise_voxel(
            voxel_grid[0].cpu().detach().numpy(),    # [10, 100, 100, 100]
            # q_attention=grad_cam_map.cpu().detach().numpy(),
            q_attention=right_q_trans[0].cpu().detach().numpy(),
            highlight_coordinate=right_coords[0].cpu().numpy(),
            highlight_gt_coordinate=right_action_trans[0].cpu().numpy(),
            voxel_size=0.045,
