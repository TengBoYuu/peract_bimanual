...
class OptionSelector(nn.Module):
    ...
    def init_fc2_weights(self):
        ...
        with torch.no_grad():  # 禁用梯度计算来避免影响模型训练
            self.fc2.weight.copy_(self.embeddings_matrix.t())  # 将权重初始化为 embeddings_matrix
    ...
    # 确保 lang_input 可以计算梯度
        ...
        # print(predicted_class) 
        y = one_hot_predicted_class.detach() + probs 
        selected_embedding_flat = self.fc2(probs)  # [1, 77*512]
