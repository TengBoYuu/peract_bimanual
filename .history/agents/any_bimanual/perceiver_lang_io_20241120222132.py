...
def symmetric_kl_divergence(left, right):
    left_prob = F.log_softmax(left, dim=-1)
    right_prob = F.log_softmax(right, dim=-1)
    
    kl_left_to_right = F.kl_div(left_prob, right_prob.exp(), reduction="batchmean")
    kl_right_to_left = F.kl_div(right_prob, left_prob.exp(), reduction="batchmean")
...
def l2_1_norm(tensor):
    l2_norm_per_skill = torch.norm(tensor, dim=-1)  # [b, num_skills]
...
class PerceiverVoxelLangEncoder(nn.Module):
    ...
    l1_norm(left) + l1_norm(right) +  # 第一部分: \|\hat{w}^{left}\|_1 + \|\hat{w}^{right}\|_1
    lambda_e * (l2_1_norm(left) + l2_1_norm(right))  # 第二部分: \lambda_e (\|\epsilon^{left}\|_{2,1} + \|\epsilon^{right}\|_{2,1})
)
