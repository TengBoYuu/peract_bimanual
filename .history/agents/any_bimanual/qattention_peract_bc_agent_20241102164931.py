...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # print(right_action_gripper_pose.shape)
        ...
        obs, pcd, rgbs, dep  = self._preprocess_inputs(replay_sample)
        ...
        q_trans_vis = False
        # print(self._right_vis_translation_qvalue.shape)
        # print(voxel_grid[0].shape)
        if step % 1 == 0 and rank == 0:
            ...
            # pcd
            # print(dep[0].shape)
            dep_src = dep[0][0].permute(1, 2, 0).cpu().numpy()
            max_value = np.max(dep_src)
            # print("最大值为:", max_value)

            ##############visdom###################


            print("Starting Visdom visualization...")
            viz = visdom.Visdom()
            assert viz.check_connection(), "Visdom server not connected. Please start the server with 'python -m visdom.server'"
            ...
            # points = np.concatenate((points, right_gt), axis=0)
            print("Points shape:", points.shape)

            # default_color = np.array([255, 0, 0])  # 红色
            # highlight_color = np.array([0, 255, 0])  # 绿色
            # default_size = 2
            # highlight_size = 10

            # # 创建颜色和大小数组
            # colors = np.array([default_color] * len(points))
            # sizes = np.array([default_size] * len(points))

            # # 判断每个点是否为 right_gt，如果是，则调整颜色和大小
            # for i, point in enumerate(points):
            #     if np.allclose(point, right_gt):  # 检查点是否与 right_gt 接近
            ...
            #     X=points,
            #     opts=dict(
            #         markersize=sizes,            # 应用大小数组
            #         markercolor=colors,          # 应用颜色数组
            ...
            # 可视化点云数据，红色
            # viz.scatter(
            #     X=right_gt.cpu().numpy().reshape(1, 3),
            #     opts=dict(
            #         markersize=20,
            #         markercolor=np.array([[0, 255, 0]]),  # 红色
            #         xlabel='X', ylabel='Y', zlabel='Z',
            #         title="Point Cloud Visualization with Right GT",
            #         markersymbol='dot'
            #     )
            # )
            # right = torch.tensor([0.5, 0.5, 0.5])  # 示例目标点
            # win = viz.scatter(
            #     X=right.cpu().numpy().reshape(1,3),  # 将点云展平为 [N, 3] 形状
            #     opts=dict(
            #         markersize=2,
            #         markercolor=np.array([[255, 0, 0]]),  # 红色
            #         xlabel='X', ylabel='Y', zlabel='Z',
            #         title="Point Cloud Visualization with Right GT",
            #         markersymbol='dot'
            #     )
            ...
            # # 追加可视化 right_gt 点（绿色，放大）
            # viz.scatter(
            #     X=right_gt.cpu().numpy().reshape(1, 3),  # 转为 [1, 3] 的形状
            #     opts=dict(
            #         markersize=10,  # 放大 right_gt 的点大小
            #         markercolor=np.array([[0, 255, 0]]),  # 绿色
            #         markersymbol='dot'
            #     ),
            #     win=win,   # 使用相同窗口 ID 追加绘制
            #     update='append'
            # )
            ...
            # 定义每个点的颜色和大小
            colors = np.array([[255, 0, 0], [0, 255, 0]])  # 红色和绿色
            sizes = np.array([2, 10])  # right 点小，right_gt 点大

            # 启动 Visdom 可视化
            viz = visdom.Visdom()
            assert viz.check_connection(), "Visdom server not connected. Please start the server with 'python -m visdom.server'"

            # 使用 scatter 绘制带有不同颜色和大小的点
            win = viz.scatter(
                X=points,
                opts=dict(
                    markersize=sizes,            # 应用每个点的大小
                    markercolor=colors,          # 应用每个点的颜色
                    xlabel='X', ylabel='Y', zlabel='Z',
                    title="Point Cloud Visualization with Right GT",
                    markersymbol='dot'
                )
            ...
            print("Visualization sent to Visdom.")
            #########################################
            # print(len(pcd)) # 1
            # print(pcd[0].shape) # [b,3,640,480]
            axs[1].imshow(dep_src)
            axs[1].title.set_text('dep')
            axs[2].imshow(rendered_img_right)
            axs[2].text(0, 40, 'predicted', color='blue')
            axs[2].text(0, 80, 'gt', color='red')
            axs[3].imshow(rendered_img_left)
            axs[3].text(0, 40, 'predicted', color='blue')
            axs[3].text(0, 80, 'gt', color='red')
            # remove axis
            for ax in axs:
                ax.axis('off')
            plt.tight_layout()
    ...
    def load_weights(self, savedir: str):
        ...
        if self.load:
            ...
            for k, v in state_dict.items():
                if self.frozen:
                    # print("PerAct weights are frozen")
                    v.requires_grad_(False)
                    # print(f"Param: {k}, requires_grad: {v.requires_grad}")
                if not self._training:
                    k = k.replace("_qnet.module", "_qnet")
                # cross_attn
