...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        if self.use_prefix:
            ...
            in_channels=self.input_preprocess.out_channels,  # 输入通道
            out_channels=self.input_preprocess.out_channels * 2,  # 输出两份相同形状的体素
            kernel_sizes=self.voxel_patch_size,  # 使用 1x1x1 卷积来控制分割方式
            strides=self.voxel_patch_stride,
            norm=None,
            activation=activation, 
            )
