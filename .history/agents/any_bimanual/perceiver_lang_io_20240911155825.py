    ...
    def forward(
        ...
        if self.predictor:
            ...
            if self.lang_fusion_type == "seq":
                print("seqqqqqqqq")
                l_right = torch.cat(self.lang_preprocess_right(right_skill),self.lang_preprocess_right(lang_token_embs))  # [B,77,512] -> [B,77,128]
                print(l_right.shape)
                ins_right = torch.cat((l_right, ins), dim=1)  # [B,8077,128]
                print(ins_right.shape)
                l_left = torch.cat(self.lang_preprocess_left(right_skill),self.lang_preprocess_right(lang_token_embs))  # [B,77,512] -> [B,77,128]
