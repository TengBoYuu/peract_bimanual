...
class QFunction(nn.Module):
    def __init__(
        ...
        if self.use_predictor:
            self.option_selector = OptionSelector(num_classes=18)
            self.option_selector.to(device)
        # distributed training
        if training:
            self._qnet = DDP(self._qnet, device_ids=[device], find_unused_parameters=True)
            if use_predictor:
                self.option_selector = DDP(self.option_selector, device_ids=[device], find_unused_parameters=True)  # 包含 option_selector 在 DDP 中
    ...
    def forward(
        ...
        # forward pass
        if self.training:
            right_lang_token_embs = None
            left_lang_token_embs = None
            print(lang_token_embs.requires_grad)
            if self.use_predictor:
                # TO DO
                # 对左右臂skill有区分
                # 加上本体感知
                # print(proprio[:,4].shape)
                right_selected_options = self.option_selector(rgbs[:4], lang_token_embs, proprio[:,:4])
                # print(right_selected_options.shape)
                left_selected_options = self.option_selector(rgbs[-4:], lang_token_embs, proprio[:,4:])
                right_lang_token_embs = torch.cat((right_selected_options, lang_token_embs),dim=1)
                left_lang_token_embs = torch.cat((left_selected_options, lang_token_embs),dim=1)
            split_pred = self._qnet(
                voxel_grid,
                proprio,
                lang_goal_emb,
                lang_token_embs,
                right_lang_token_embs,
                left_lang_token_embs,
            ...
            if use_skill:
                option_selector = OptionSelector(num_classes=18)
                selected_options = option_selector(rgb,lang_token_embs)
...
class QAttentionPerActBCAgent(Agent):
    ...
    def act(self, step: int,observation: dict, train_cfg: dict,deterministic=False) -> ActResult:
        deterministic = True
        ...
        (
            ...
            prev_layer_bounds,
            prev_layer_voxel_grid,
            train_cfg.framework.use_predictor,
            train_cfg.rlbench.instructions
        )
    ...
    def load_weights(self, savedir: str):
        ...
        for k, v in state_dict.items():
            ...
            # pos_with_lang
                ...
                if v.shape[1] != 8077 or v.shape[1] != 8078:
                    if self.use_predictor:
                        lang_max_seq_len = 78
                    else:
                        lang_max_seq_len = 77
