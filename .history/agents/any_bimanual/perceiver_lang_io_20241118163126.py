...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        if self.low_dim_size > 0:
            ...
            if self.use_skill:
                self.proprio_preprocess_right = DenseBlock(
                    int(self.low_dim_size/2),
                    self.im_channels,
                    norm=None,
                    activation=activation,
                )
                self.proprio_preprocess_left = DenseBlock(
                    int(self.low_dim_size/2),
                    self.im_channels,
                    norm=None,
                    activation=activation,
    ...
    def forward(
        ...
        if self.use_skill:
            skill_ins_right = rearrange(skill_ins_right, "b d ... -> b ... d")  # [B,20,20,20,128]
            skill_ins_left = rearrange(skill_ins_left, "b d ... -> b ... d")  # [B,20,20,20,128]
        ...
        if self.use_skill:
            skill_ins_right = rearrange(skill_ins_right, "b ... d -> b (...) d")  # [B,8000,128]
            skill_ins_left = rearrange(skill_ins_left, "b ... d -> b (...) d")  # [B,8000,128]
        ...
        if self.use_skill:
            if self.lang_fusion_type == "seq":
                ...
                # right_skill, left_skill = self.option_selector(skill_ins_right, skill_ins_left,l) # 

                right_skill = self.option_selector(skill_ins_left,l)
                left_skill = self.option_selector(skill_ins_left,l)
                right_skill = self.lang_preprocess(right_skill)
                left_skill = self.lang_preprocess(left_skill)
                right_skill = right_skill + l
                left_skill = left_skill + l
                ...
                ins_right = torch.cat((l_right, ins), dim=1)  # [B,8154,128]
                l_left = torch.cat((left_skill, l), dim=1)
                ins_left = torch.cat((l_left, ins), dim=1)  # [B,8154,128]
        ...
        if self.use_prefix:
            ...
            encode_voxel_right =  mask_right[0,77:,:]
            encode_voxel_left =  mask_left[0,77:,:]
            # vis_voxel(encode_voxel_right)
            vis_voxel(encode_voxel_left)
            ...
            # print(ins.shape) # [B, 8077, 128]
            ins_r = torch.cat((ins, right_skill),dim=1)
            ins_l = torch.cat((ins, left_skill), dim=1)
            ins_right = torch.cat((ins_right, ins_r), dim=2)
            ins_left = torch.cat((ins_left, ins_l), dim=2)
            # print(ins_right.shape) # 256
...
def get_position_based_colors(positions, n_components=3):
    """Generate colors based on positions using PCA-like transformation."""
    # Standardize the positions
    scaler = StandardScaler()
    positions_scaled = scaler.fit_transform(positions)
    
    # Apply PCA to get the main components for coloring
    pca = PCA(n_components=n_components)
    position_features = pca.fit_transform(positions_scaled)
    
    # Normalize to [0, 1] range for colors
    colors = (position_features - position_features.min(axis=0)) / (position_features.max(axis=0) - position_features.min(axis=0))
    return colors

def plot_voxelized_pointcloud_with_voxel_data(voxel, voxel_size=0.05, save_path="voxelized_plot.png", dpi=300):
    # Ensure voxel data is a numpy array
    if isinstance(voxel, torch.Tensor):
        voxel = voxel.cpu().numpy()
    
    # Apply PCA to reduce dimensions from 128 to 3 for color representation
    pca = PCA(n_components=3)
    reduced_data = pca.fit_transform(voxel)

    # Normalize reduced data to [0, 1] for color mapping
    reduced_data_min = reduced_data.min(axis=0)
    reduced_data_max = reduced_data.max(axis=0)
    colors = (reduced_data - reduced_data_min) / (reduced_data_max - reduced_data_min)

    # Increase brightness if needed
    brightness_factor = 1.5
    colors = np.clip(colors * brightness_factor, 0, 1)

    # Generate voxel positions by normalizing and scaling to fit within the voxel grid
    positions_min = voxel.min(axis=0)
    positions_max = voxel.max(axis=0)
    voxel_points = (voxel - positions_min) / (positions_max - positions_min)
    voxel_points = (voxel_points / voxel_size).astype(int)

    # Get unique voxel positions and assign colors based on PCA components
    unique_voxels = np.unique(voxel_points, axis=0)
    voxel_colors = get_position_based_colors(unique_voxels)

    # Scale up colors for better visibility and clip to [0, 1]
    voxel_colors = np.clip(voxel_colors * 3.0, 0, 1)
    ...
    # Plot voxels as cubes with colors based on PCA
    for voxel, color in zip(unique_voxels, voxel_colors):
        ax.bar3d(voxel[0] * voxel_size, 
                 voxel[1] * voxel_size, 
                 voxel[2] * voxel_size,
    ...
    plt.close()
    print(f"Saved voxelized plot to {save_path}")

def vis_voxel(voxel):
    plot_voxelized_pointcloud_with_voxel_data(voxel, voxel_size=0.05, save_path="/mnt/disk_1/tengbo/peract_bimanual/misc/voxelized_plot.png", dpi=300)