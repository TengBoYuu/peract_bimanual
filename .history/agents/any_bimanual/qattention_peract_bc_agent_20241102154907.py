...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        if step % 1 == 0 and rank == 0:
            ...
            # 添加 right_gt 点并设置颜色
            all_points = np.vstack((points, right_gt.cpu().numpy()))
            colors = np.array([[255, 0, 0]] * len(points) + [[0, 255, 0]])  # 红色为点云，绿色为 right_gt
            markersizes = np.array([2] * len(points) + [5])  # 设定 right_gt 的点大小更大
