...
class QFunction(nn.Module):
    def __init__(
        ...
        self.option_selector = OptionSelector(num_classes=18, device_ids=[device])
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.use_predictor:
                ...
                right_selected_options = self.option_selector(rgb[:4], lang_token_embs, proprio[:,:4])
                # print(right_selected_options.shape)
                left_selected_options = self.option_selector(rgb[-4:], lang_token_embs, proprio[:,4:])
