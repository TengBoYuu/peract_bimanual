...
class OptionSelector(nn.Module):
    def __init__(self, num_classes, embedding_matrix):
        ...
        self.conv1 = nn.Conv1d(in_channels=8077, out_channels=1024, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, ins, lang):
        ...
        ins_pooled = F.adaptive_avg_pool1d(ins_.permute(0, 2, 1), output_size=256)  # Pool to [B, 128, 256]
        ins_flat = ins_pooled.reshape(ins_pooled.size(0), -1)  # Flatten to [B, 128*256]
