...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        if self.predictor:
            self.latents = nn.Parameter(torch.randn(num_latents/2, latent_dim))
        else:
        ...
        self.cross_attend_blocks_with_skill = nn.ModuleList(
            [
                PreNorm(
                    latent_dim,
                    Attention(
    ...
    def forward(
        ...
        if self.predictor:
            ...
            cross_attn_right, cross_attn_left, cross_ff_right, cross_ff_left = self.cross_attend_blocks_with_skill
        else:
        ...
        for it in range(self.iterations):
            ...
            if self.predictor:
                print("skill")
                x_right = cross_attn(x_r, context=ins_right, mask=mask) + x_r
                x_left = cross_attn(x_l, context=ins_left, mask=mask) + x_l
            else:
        ...
        if self.predictor:
            ...
            latents_left = self.decoder_cross_attn_left(ins_right, context=x_left)
        else:
