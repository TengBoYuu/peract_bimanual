...
from agents.any_bimanual.perceiver_lang_io import PerceiverVoxelLangEncoder
from agents.any_bimanual.qattention_peract_bc_agent import QAttentionPerActBCAgent
from agents.any_bimanual.qattention_stack_agent import QAttentionStackAgent
from agents.any_bimanual.option_selector1 import OptionSelector
from omegaconf import DictConfig

# any_bimanual 从这里创建机器人的，因为左右臂同时出，不需要单独创建，只需要用一个函数里面创建就好了
def create_agent(cfg: DictConfig):
    ...
    for depth, vox_size in enumerate(cfg.method.voxel_sizes):
        ...
        perceiver_encoder = PerceiverVoxelLangEncoder(
            ...
            predictor = cfg.framework.use_predictor,
            prefix = cfg.framework.use_prefix,
        )
