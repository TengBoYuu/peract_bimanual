...
def vis_voxel(voxel):
    # 将数据重塑为二维数组，确保数据是 NumPy 格式
    reshaped_data = voxel
    if isinstance(reshaped_data, torch.Tensor):
        if reshaped_data.is_cuda:
            reshaped_data = reshaped_data.cpu().numpy()
        else:
            reshaped_data = reshaped_data.numpy()
    else:
    ...
    # 将降维后的数据归一化到 [0, 1]
    reduced_data_min = reduced_data.min()
    reduced_data_max = reduced_data.max()
    colors = (reduced_data - reduced_data_min) / (reduced_data_max - reduced_data_min)

    # 增亮颜色值
    brightness_factor = 1.5  # 可以尝试更高的值，比如 2.0
    colors[:, :3] *= brightness_factor
    colors = np.clip(colors, 0, 1)

    # 添加 Alpha 通道
    alpha_channel = np.ones((colors.shape[0], 1)) * 0.9
    colors = np.concatenate((colors, alpha_channel), axis=1)

    # 将颜色数组转换为颜色元组列表
    colors_tuple = [tuple(c) for c in colors]

    # 创建体素数据和 facecolors 数组
    voxel_data = np.ones((20, 20, 20), dtype=bool)
    facecolors = np.empty(voxel_data.shape, dtype=object)
    facecolors.ravel()[:] = colors_tuple

    # 过滤接近黑色的体素块
    black_threshold = 0.1  # 可以调节这个值，越低表示更接近黑色
    for i in range(facecolors.shape[0]):
        for j in range(facecolors.shape[1]):
            for k in range(facecolors.shape[2]):
                color = facecolors[i, j, k]
                if color[:3] <= (black_threshold, black_threshold, black_threshold) and color[3] == 0.9:
                    voxel_data[i, j, k] = False  # 隐藏接近黑色的体素
    ...
    # 绘制体素，隐藏黑色体素块，去掉边框
    ax.voxels(voxel_data, facecolors=facecolors, edgecolor=None)

    # 保存图像
    save_path = "/mnt/disk_1/tengbo/peract_bimanual/misc/voxelized_plot.png"
