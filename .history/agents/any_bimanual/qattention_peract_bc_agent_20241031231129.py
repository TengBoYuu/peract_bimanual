...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # print(right_q_trans[0].shape) # [1,100,100,100] # 对每个网格的注意力权重
        ...
        # pcd
        dep_src = dep[0].squeeze(0).permute(1, 2, 0).cpu().numpy()
        ...
        # # 生成点云数据
        # point_cloud = pcd[0].squeeze(0)  # 去掉批次维度，变为 [3, 640, 480]
        # x, y, z = point_cloud[0].view(-1), point_cloud[1].view(-1), point_cloud[2].view(-1)
        # points = torch.stack((x, y, z), dim=1).cpu().numpy()  # 转换为 numpy 数组
        ...
        #     opts=dict(
        #         markersize=2,
        #         markercolor=np.array([[255, 0, 0]]),  # 单一红色
