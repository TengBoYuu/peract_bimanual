...
class QFunction(nn.Module):
    def __init__(
        ...
        self.option_selector = option_selector.to(device)
        # distributed training
        if training:
            self._qnet = DDP(self._qnet, device_ids=[device], find_unused_parameters=False)
            if use_predictor:
                self.option_selector = DDP(self.option_selector, device_ids=[device], find_unused_parameters=False, broadcast_buffers=False)  # 包含 option_selector 在 DDP 中
    ...
    def forward(
        ...
        if self.training:
            ...
            if self.use_predictor:
                ...
                right_skill = self.option_selector(rgbs[:4], lang_token_embs, proprio[:,:4])
                right_lang_token_embs = torch.cat((right_skill, lang_token_embs),dim=1)
                # print(right_selected_options.shape)
                left_skill = self.option_selector(rgbs[-4:], lang_token_embs, proprio[:,4:])
...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # 打印 optimizer 中包含的所有参数的名称、requires_grad 和形状
        # 获取所有命名的参数和优化器中的参数进行匹配
        # named_params = dict(self._q.named_parameters())
        ...
        #                 break
        # 打印 optimizer 中包含的所有参数的名称
        # for name, param in self._q.named_parameters():
        #     if param in self._optimizer.param_groups[0]['params']:
        #         print(f"Parameter name: {name}, requires_grad: {param.requires_grad}, shape: {param.shape}")
