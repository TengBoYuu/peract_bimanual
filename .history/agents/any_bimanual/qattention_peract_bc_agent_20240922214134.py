...
class QAttentionPerActBCAgent(Agent):
    ...
    def concat_weights(self, param, target_size):
        if param.size(-1) < target_size:
            param = torch.cat([param, param], dim=-1)
        return param
    def load_weights(self, savedir: str):
        device = (
            self._device
                ...
                if k.startswith("_qnet.module.decoder_cross_attn"):
                    ...
                    if self.use_prefix:
                        ...
                        if v.size(-1) == 128:
                            merged_state_dict[right_key] = self.concat_weights(v, 256)
                            merged_state_dict[left_key] = self.concat_weights(v, 256)
                if k == "_qnet.module.up0.conv_up.0.conv3d.weight":
                    # 对conv3d权重进行处理，拼接第二维度
                    if v.size(1) == 128:  # 检查是否为128维
                        merged_state_dict[k] = concat_conv_weights(v, 256, dim=1)
                    else:
                        merged_state_dict[k] = v     
                # trans_decoder
