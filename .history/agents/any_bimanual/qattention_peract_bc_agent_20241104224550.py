...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        if self._transform_augmentation:
            ...
            # for se3 debugging
            # pcd_save = pcd[0].detach().cpu().numpy()
            ...
            # debug_path = os.path.join("/mnt/disk_1/tengbo/peract_bimanual-real/voxel/", "debug")
            ...
            from voxel import augmentation, my_augmentation
            (
                right_action_trans,
                ...
                pcd,
            ) = my_augmentation.bimanual_apply_se3_augmentation(
                pcd,
        ...
        # print("pcd: ",pcd[0].shape)
        # print("lang: ", lang_token_embs.shape)
        ...
        if self.cfg.framework.use_wandb and step % 10 == 0 and rank == 0:
            wandb.log({
        ...
        if step % self.cfg.framework.log_freq == 0 and rank == 0:
            ...
            #     marker=dict(
            #         size=10,
            #         color='green'  # 设置 right_gt 点颜色为绿色
            #     ),
            #     name='Right GT'
            # ))
            ...
            # # 保存图表为 HTML 文件
            # # 自动递增文件名
            # base_path = "/mnt/disk_1/tengbo/peract_bimanual-real/pcd/3d_scatter_plot_"
            # x = 1
            # while True:
