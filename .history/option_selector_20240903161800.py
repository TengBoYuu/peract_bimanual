...
from utils.option_utils import pad, entropy
...
class OptionSelector(nn.Module):
    ...
    def forward(self, word_embeddings, states, attention_mask=None, **kwargs):
        if self.method == 'traj_option':
            dt_ret = self.option_dt(word_embeddings, states, attention_mask)
    ...
    def get_option(self, word_embeddings, states, **kwargs):
        ...
        if self.method == 'traj_option':
            ...
            options, option_indx, _, _, _ = self.forward(
                word_embeddings, states, attention_mask=attention_mask, **kwargs)
        else:
