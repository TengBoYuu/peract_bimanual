...
# 设置y轴标签和标题
ax.set_ylabel('Success Rates', fontsize=10, labelpad=10)  # 加大labelpad以确保显示完整
ax.set_title('RLBench2', fontsize=12)

# 在柱子上方标注平均值，稍微减少偏移量，避免超出太多
for bar, mean, err in zip(bars, means, std_errors):
    height = bar.get_height()
    ax.annotate(f'{mean:.2f}', xy=(bar.get_x() + bar.get_width() / 2, height + err * 0.5),  # 控制位置偏移，减少偏移量
                xytext=(0, 3),  # 位置偏移
