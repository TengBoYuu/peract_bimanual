...
class OptionTransformer(nn.Module):
    ...
    def forward(self, word_embeddings, states, attention_mask, **kwargs):
        ...
        state_embeddings = self.embed_state(states).to(states.device)
        ret_state_embeddings = state_embeddings.clone().detach()
        lang_embeddings = self.embed_lang(word_embeddings).to(states.device)
        print('----------------------emb-------------------', state_embeddings.shape)
        # Remove time embeddings addition since timesteps are not used
        lang_and_inputs = torch.cat([lang_embeddings, state_embeddings], dim=1).to(states.device)
        # LAYERNORM AFTER LANGUAGE
        stacked_inputs = self.embed_ln(lang_and_inputs).to(states.device)
