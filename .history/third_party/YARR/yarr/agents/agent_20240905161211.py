...
class Agent(ABC):
    ...
    @abstractmethod
    def act(self, step: int, observation: dict, train_cfg: dict, deterministic: bool) -> ActResult:
...
class BimanualAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        for k, v in replay_sample.items(): 
            if "rgb" in k or "point_cloud" in k or "camera" in k: # 左右臂都能看到全局信息
    ...
    def act(self, step: int, observation: dict, deterministic: bool, train_cfg) -> ActResult:
        ...
        right_act_result = self.right_agent.act(step, right_observation, deterministic, train_cfg)
        left_act_result = self.left_agent.act(step, left_observation, deterministic, train_cfg)
        # print("------right_act_result-------") # 这里也已经是连续的了
        # print(right_act_result.action)
...
class LeaderFollowerAgent(Agent):
    ...
    def act(self, step: int, observation: dict, deterministic: bool, train_cfg: dict) -> ActResult:
        ...
        right_act_result = self.leader_agent.act(step, leader_observation, deterministic,train_cfg)
        ...
        left_act_result = self.follower_agent.act(step, follower_observation, deterministic,train_cfg)
