...
class _IndependentEnvRunner(_EnvRunner):
    ...
    def _run_eval_independent(self, name: str,
        ...
        for n_eval in tqdm(range(self._num_eval_runs),mininterval=10, desc="Processing files"):
            if rec_cfg.enabled:
            ...
            # 每一个episodes
            for ep in tqdm(range(self._eval_episodes),desc="Processing files"):
                ...
                # print("--------generator-----------")
                try:
                    # 每一个replay_transition先out再进循环
                    for replay_transition in generator:
                        # print("---------replay transition---------")
                        # print(replay_transition)
                        while True:
