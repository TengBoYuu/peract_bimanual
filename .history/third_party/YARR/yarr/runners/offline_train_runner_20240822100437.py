...
class OfflineTrainRunner(BaseTrainTester):
    ...
    def _get_datasets(self,instructions,tasks):
        """Initialize datasets."""
    ...
    def pad_tensors(self, tensors, pad_value=0):
        # 找到最长的序列长度
        max_len = max(tensor.shape[0] for tensor in tensors)
        padded_tensors = []
        
        for tensor in tensors:
            padding_size = (max_len - tensor.shape[0],) + tuple(0 for _ in tensor.shape[1:])
            padded_tensor = torch.nn.functional.pad(tensor, padding=padding_size, value=pad_value)
    ...
    def custom_collate_fn(self,batch):
        ...
        for key in batch[0].keys():
            if key in ['right_rgbs', 'left_rgbs', 'right_pcds', 'left_pcds',
                ...
                # 对每个需要处理的键进行填充处理
                batch_dict[key] = self.pad_tensors([item[key] for item in batch])
            else:
                # 直接堆叠其他类型的数据
    ...
    def start(self):
        ...
        if self.method.startswith("DIFFUSER_ACTOR"):
            ...
            dataset = self._get_datasets(self.instructions,self.tasks)
            for i, data in enumerate(dataset):
                print(f"Sample {i} shapes:")
                for key, value in data.items():
                    print(f"{key}: {value.shape if isinstance(value, torch.Tensor) else type(value)}")
            data_loader = self.get_loaders(dataset,collate_fn=self.custom_collate_fn)
