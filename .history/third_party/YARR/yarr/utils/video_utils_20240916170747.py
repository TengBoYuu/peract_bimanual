...
class TaskRecorder(object):
    ...
    def save(self, path, lang_goal, reward, skill=None):
        ...
        if skill:
            skill_textsize = cv2.getTextSize(skill, font, font_scale, font_thickness)[0]
            skill_textX = (image_size[0] - skill_textsize[0]) // 2

            frame = cv2.putText(frame, skill, org=(skill_textX, image_size[1] - 70),  # Display above lang_goal
                                fontScale=font_scale, fontFace=font, color=(0, 0, 0),
                                thickness=font_thickness, lineType=cv2.LINE_AA)
            video.write(frame)
        video.release()
