        ...
        for i in range(num_steps):
            ...
            for camera_name, camera_config in obs_config.camera_configs.items():                
                ...
                if camera_config.rgb:
                    ...
                    # image = np.array((Image.open(image_path), camera_config.image_size))
                ...
                if camera_config.depth or camera_config.point_cloud:
                    ...
                    # image = image_to_float_array( (Image.open(image_path), camera_config.image_size),DEPTH_SCALE)

                    # print(f"image.min: {image.min()}, image.max: {image.max()}, how much max: {image[image == 1].shape[0]}") 
                    # image[image == image.max()] = 0

                    if camera_config.depth:
                        if camera_config.depth_in_meters:
                            near = obs[i].misc[f'{camera_name}_camera_near']
                            print("depth_near: ",near)
                            # near = 0
                    ...
                    if camera_config.point_cloud:
                        ...
                        # print(f"image.min: {depth_image_m.min()}, image.max: {depth_image_m.max()}, how much max: {depth_image_m[depth_image_m == 1].shape[0]}") 
                        # max_ = depth_image_m.max()
                        depth_image_m[depth_image_m > 1.19 ] = 0
                        # print("image.max:",depth_image_m.max())
                ...
                if camera_config.mask:
                    ...
                    # obs[i].perception_data[f"{camera_name}_mask"] = rgb_handles_to_mask(np.array((Image.open(image_path), camera_config.image_size)))
