...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> None:
        ...
        state_dict[2].extend(left_keyframe_action_ls[1:]) # left action
        state_dict[3].extend(attn_indices)
        state_dict[4].extend(right_keyframe_action_ls[:-1])  # right gripper pos
        state_dict[4].extend(left_keyframe_action_ls[:-1])  # left gripper pos
        state_dict[5].extend(right_intermediate_action_ls)   # traj from gripper pos to keyframe action
        state_dict[5].extend(left_intermediate_action_ls)   # traj from gripper pos to keyframe action
