...
def get_attn_indices_from_demo(
    ...
    frames.insert(0, 0)
    if Arm == 'right':
        return [{cam: obs_to_attn_right(demo[f], cam) for cam in cameras} for f in frames]
    else:
        return [{cam: obs_to_attn_left(demo[f], cam) for cam in cameras} for f in frames]
...
def get_observation(task_str: str, variation: int,
    ...
    for i in range(len(key_frame)):
        state, right_action, left_action = env.get_obs_action(demo._observations[key_frame[i]]);
        ...
        if store_intermediate_actions and i < len(key_frame) - 1:
            ...
            for j in range(key_frame[i], key_frame[i + 1] + 1):
                _, right_action, left_action = env.get_obs_action(demo._observations[j])
    ...
    return demo, keyframe_state_ls, right_keyframe_action_ls, left_keyframe_action_ls, right_intermediate_action_ls, left_intermediate_action_ls
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> None:
        ...
        right_attn_indices = get_attn_indices_from_demo(task, demo, args.cameras)
        left_attn_indices = get_attn_indices_from_demo(task, demo, args.cameras)
        ...
        # bimanual
        state_dict: List = [[] for _ in range(9)]
        print("Demo {}".format(episode))
        state_dict[0].extend(frame_ids)
        state_dict[1] = state_ls[:-1].numpy()
        state_dict[2].extend(right_keyframe_action_ls[1:]) # right action
        state_dict[3].extend(left_keyframe_action_ls[1:]) # left action
        state_dict[4].extend(attn_indices)
        state_dict[5].extend(right_keyframe_action_ls[:-1])  # right gripper pos
        state_dict[6].extend(left_keyframe_action_ls[:-1])  # left gripper pos
        state_dict[7].extend(right_intermediate_action_ls)   # traj from gripper pos to keyframe action
        state_dict[8].extend(left_intermediate_action_ls)   # traj from gripper pos to keyframe action
