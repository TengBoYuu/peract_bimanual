...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> None:
        ...
        right_frame_ids = list(range(len(left_state_ls) - 1))
        ...
        # bimanual
        state_dict: List = [[] for _ in range(11)]
        print("Demo {}".format(episode))
        state_dict[0].extend(frame_ids)
        state_dict[1] = right_state_ls[:-1].numpy()
        state_dict[2] = left_state_ls[:-1].numpy()
        state_dict[3].extend(right_keyframe_action_ls[1:]) # right action
        state_dict[4].extend(left_keyframe_action_ls[1:]) # left action
        state_dict[5].extend(right_attn_indices)
        state_dict[6].extend(left_attn_indices)
        state_dict[7].extend(right_keyframe_action_ls[:-1])  # right gripper pos
        state_dict[8].extend(left_keyframe_action_ls[:-1])  # left gripper pos
        state_dict[9].extend(right_intermediate_action_ls)   # traj from gripper pos to keyframe action
        state_dict[10].extend(left_intermediate_action_ls)   # traj from gripper pos to keyframe action
