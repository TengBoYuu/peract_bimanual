...
def get_observation(task_str: str, variation: int,
    ...
    for i in range(len(key_frame)):
        ...
        left_state, right_action = env.get_obs_action_left(demo._observations[key_frame[i]])
        ...
        left_keyframe_state_ls.append(right_state.unsqueeze(0))
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> None:
        ...
        # bimanual
        state_dict: List = [[] for _ in range(10)]
        print("Demo {}".format(episode))
        state_dict[0].extend(frame_ids)
        state_dict[1] = state_ls[:-1].numpy()
        ...
        state_dict[4].extend(right_attn_indices)
        state_dict[5].extend(left_attn_indices)
        state_dict[6].extend(right_keyframe_action_ls[:-1])  # right gripper pos
        state_dict[7].extend(left_keyframe_action_ls[:-1])  # left gripper pos
        state_dict[8].extend(right_intermediate_action_ls)   # traj from gripper pos to keyframe action
        state_dict[9].extend(left_intermediate_action_ls)   # traj from gripper pos to keyframe action
