...
class OptionSelector(nn.Module):  # 继承自 torch.nn.Module
    def __init__(self, num_classes=18, vocab=vocabulary, device_ids=None):
        ...
        # 如果没有指定device，则默认使用CPU或可用的CUDA设备
        if device_ids is None:
            self.device = "cuda" if torch.cuda.is_available() else "cpu"
        else:
            self.device = device_ids[0]  # 从device_ids中选取第一个设备
        self.clip_model, _ = clip.load("RN50", device=self.device, jit=False)
    ...
    def forward(self, rgb_list, lang_input):
        ...
        max_class_idx = predicted_class.item()  # 假设只输出一个类别的嵌入
        vocab_key = list(self.vocabulary.keys())[max_class_idx]
        sentences = self.vocabulary[vocab_key]  # 获取该类别下所有句子
        ...
        for sentence in sentences:
            # 使用CLIP将文本转换为嵌入
            text_tokens = clip.tokenize([sentence]).to(self.device)  # 与设备对齐
            text_embedding = self.clip_model.encode_text(text_tokens)
            sentence_embeddings.append(text_embedding)

        # 将所有句子的嵌入合并，并计算平均值
        sentence_embeddings = torch.stack(sentence_embeddings, dim=0)  # [num_sentences, embed_dim]
        mean_embedding = sentence_embeddings.mean(dim=0)  # [embed_dim]

        # 对齐到与语言指令相同的维度 [b, 77, 512]
        mean_embedding = mean_embedding.unsqueeze(1).expand(-1, lang_input.size(1), -1)
