...
def pad_and_crop_tensors(tensors, target_size=128, pad_value=0):
    padded_tensors = []
    
    for tensor in tensors:
        # 首先处理高度 (dim=3) 和宽度 (dim=4)
        ...
        # 如果高度小于目标大小，进行填充
            ...
            tensor = torch.nn.functional.pad(tensor, (0, 0, 0, pad_height), value=pad_value)
        ...
        # 如果宽度小于目标大小，进行填充
            ...
            tensor = torch.nn.functional.pad(tensor, (0, pad_width, 0, 0), value=pad_value)
    ...
    for key in batch[0].keys():
        ...
        if isinstance(first_item, torch.Tensor):
            ...
            if key in ['right_rgbs', 'left_rgbs', 'right_pcds', 'left_pcds',
                ...
                # 对每个需要处理的键进行填充处理
                batch_dict[key] = pad_and_crop_tensors([item[key] for item in batch],target_size=128)
            else:
                # 直接堆叠 Tensors
