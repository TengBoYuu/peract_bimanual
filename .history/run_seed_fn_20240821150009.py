...
class ReplayBuffer:
    def __init__(self):
        # 初始化 replay buffer 用于保存 train 和 test 数据集
        self.datasets = {
            'train': [],
            'test': []
        }

    def add_dataset(self, dataset, dataset_type='train'):
        # 根据 dataset_type 来添加数据集
        if dataset_type not in self.datasets:
            raise ValueError("Invalid dataset type. Expected 'train' or 'test'.")
        
        # 将数据集添加到指定类型的缓冲区
        self.datasets[dataset_type].extend(dataset)
