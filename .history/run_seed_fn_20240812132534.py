...
def compare_terminal_data(buffer1, buffer2):
    data1 = buffer1._store['terminal']
    data2 = buffer2._store['terminal']
    
    if data1.shape != data2.shape:
        print(f"Shape mismatch: Buffer1 shape {data1.shape}, Buffer2 shape {data2.shape}")
    else:
        mismatches = np.where(data1 != data2)
        if mismatches[0].size > 0:
            print(f"Found {len(mismatches[0])} mismatches in 'terminal' data:")
            for index in zip(*mismatches):
                print(f"Mismatch at index {index}: Buffer1 value {data1[index]}, Buffer2 value {data2[index]}")
        else:
            print("'terminal' data is identical in both buffers.")
