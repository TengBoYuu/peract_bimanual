...
def pad_tensors(tensors, pad_value=0):
    ...
    for tensor in tensors:
        # 计算需要填充的维度，torch.nn.functional.pad 需要一个扁平的 pad 参数列表
        padding_size = [0] * (len(tensor.shape) - 1) * 2  # 每个维度前后都不需要填充
        padding_size[0] = max_len - tensor.shape[0]  # 只在第一个维度上填充
        
        padded_tensor = torch.nn.functional.pad(tensor, pad=padding_size, value=pad_value)
