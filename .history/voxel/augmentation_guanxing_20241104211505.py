...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            """
            # Subtract the center of rotation from the matrix's translation part
            matrix_translation = matrix[:, :3, 3]
            matrix_centered = matrix.clone()
            matrix_centered[:, :3, 3] = matrix_translation - C

            # Apply rotation
            rotation_homogeneous = torch.cat([rotation_matrix, torch.zeros_like(translation[:, :1, :])], dim=2)
            rotation_homogeneous = torch.cat([rotation_homogeneous, torch.zeros_like(translation[:, 1:, :])], dim=1)
            rotation_homogeneous[:, 3, 3] = 1

            # Perform matrix multiplication
            rotated_matrix = torch.matmul(rotation_homogeneous, matrix_centered)

            # Apply translation
            final_matrix = rotated_matrix.clone()
            final_matrix[:, :3, 3] = rotated_matrix[:, :3, 3] + translation + C
