...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def translate_rotate_matrices(A, C, translation, rotation_matrix):
            # Translate matrices to origin
            translate_to_origin = np.eye(4)
            translate_to_origin[:3, 3] = -C

            # Translate back from origin
            translate_back = np.eye(4)
            translate_back[:3, 3] = C
            ...
            transformation = translate_back @ rotation_matrix @ translate_to_origin @ translation_matrix

            # Apply transformation to both A and B
            A_transformed = transformation @ A
        ...
        right_perturbed_action_gripper_4x4 = translate_rotate_matrices(
            right_action_gripper_4x4.cpu().numpy(),
            center_action.cpu().numpy(),
            trans_shift,
            rot_shift_4x4.cpu().numpy()
        )
