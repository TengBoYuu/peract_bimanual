...
def bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        right_perturbed_trans = torch.tensor(np.array(right_trans_indices)).to(
            device=device, dtype=torch.int32
        )
        right_perturbed_rot_grip = torch.tensor(np.array(right_rot_grip_indices)).to(
            device=device, dtype=torch.int32
        )
        left_perturbed_trans = torch.tensor(np.array(left_trans_indices)).to(
            device=device, dtype=torch.int32
        )
        left_perturbed_rot_grip = torch.tensor(np.array(left_rot_grip_indices)).to(
            device=device, dtype=torch.int32
        )
