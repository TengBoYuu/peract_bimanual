...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            T_neg_C = torch.eye(4, device=device).unsqueeze(0).repeat(B, 1, 1)
            T_neg_C[:, :3, 3] = -C.squeeze(-1)

            R = torch.eye(4, device=device).unsqueeze(0).repeat(B, 1, 1)
            R[:, :3, :3] = rotation_matrix

            T_C = torch.eye(4, device=device).unsqueeze(0).repeat(B, 1, 1)
            T_C[:, :3, 3] = C.squeeze(-1)

            T_translation = torch.eye(4, device=device).unsqueeze(0).repeat(B, 1, 1)
            T_translation[:, :3, 3] = translation.squeeze(-1)

            # Combine transformations: T = T_translation * T_C * R * T_neg_C
            transform_matrix = T_translation @ T_C @ R @ T_neg_C
