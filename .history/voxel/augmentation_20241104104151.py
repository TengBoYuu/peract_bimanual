...
def bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        for b in range(bs):
            ...
            # Right arm
            right_trans_idx = point_to_voxel_index(
            ...
            right_quat = normalize_quaternion(right_quat)
            if right_quat[-1] < 0:
                right_quat = -right_quat
            right_disc_rot = quaternion_to_discrete_euler(
            ...
            # Left arm
            left_trans_idx = point_to_voxel_index(
            ...
            left_quat = normalize_quaternion(left_quat)
            if left_quat[-1] < 0:
                left_quat = -left_quat
            left_disc_rot = quaternion_to_discrete_euler(
