...
if __name__ == "__main__":
    ...
    # test bimanual_apply_se3_augmentation
    # Sample inputs
    batch_size = 1
    pcd = [torch.rand(batch_size, 3, 128, 128) for _ in range(1)]  # Random point clouds
    right_action_gripper_pose = torch.rand(batch_size, 7)  # Random poses
    ...
    # bounds = torch.tensor([[0.0,-1.0,-0.2,1.0,0.0,0.8] for _ in range(batch_size)])
    bounds = torch.tensor([[0.0, 0.0, 0.0, 1.0, 1.0, 1.0]])
    layer = 0
    trans_aug_range = torch.tensor([0.125, 0.125, 0.125])
    # trans_aug_range = [0.125, 0.125, 0.125]
    rot_aug_range = [0.0, 0.0, 45.0]  # Degrees
    rot_aug_resolution = 5  # Degrees
    voxel_size = [100]
