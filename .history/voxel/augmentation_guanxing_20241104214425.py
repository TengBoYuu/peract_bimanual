...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        left_perturbed_action_gripper_4x4[:3, 3] -= center_action.squeeze()
        left_perturbed_action_gripper_4x4[:3, 3] = torch.matmul(rot_shift_3x3.squeeze(), left_perturbed_action_gripper_4x4[:3, 3])
        left_perturbed_action_gripper_4x4[:3, 3] += center_action.squeeze()
        left_perturbed_action_gripper_4x4[:3, :3] = torch.matmul(rot_shift_3x3, left_perturbed_action_gripper_4x4[:3, :3])
