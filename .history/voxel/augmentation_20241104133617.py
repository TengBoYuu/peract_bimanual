...
if __name__ == "__main__":
    ...
    pcd_flat = torch.zeros(batch_size, 3, 100, 100)
    x = torch.linspace(0.0, 1.0, steps=100)
    y = torch.linspace(0.0, 1.0, steps=100)
    ...
    pcd = [pcd_flat]
    right_action_gripper_pose = torch.tensor([[0.0, 0.0, 0.5, 1.0, 0.0, 0.0, 0.0]])
    right_action_trans = right_action_gripper_pose[:, :3]
    right_action_rot_grip = right_action_gripper_pose[:, 3:]
    left_action_gripper_pose = torch.tensor([[0.0, 0.0, 0.5, 1.0, 0.0, 0.0, 0.0]])
