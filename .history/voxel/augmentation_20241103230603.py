...
def bimanual_apply_se3_augmentation(
    ...
    # Define the average center as the rotation center
    average_action_gripper_4x4 = (right_action_gripper_4x4 + left_action_gripper_4x4) / 2

    # Separate rotations for right and left grippers
    right_rotated_action_gripper_4x4 = torch.bmm(right_action_gripper_4x4, rot_shift_4x4)
    left_rotated_action_gripper_4x4 = torch.bmm(left_action_gripper_4x4, rot_shift_4x4)

    # Apply translation centered at the average for each gripper
    right_perturbed_action_gripper_4x4 = right_rotated_action_gripper_4x4.clone()
    right_perturbed_action_gripper_4x4[:, 0:3, 3] += (average_action_gripper_4x4[:, 0:3, 3] + trans_shift)

    left_perturbed_action_gripper_4x4 = left_rotated_action_gripper_4x4.clone()
    left_perturbed_action_gripper_4x4[:, 0:3, 3] += (average_action_gripper_4x4[:, 0:3, 3] + trans_shift)

    # Convert transformation matrices to translations + quaternions for both grippers
    right_perturbed_action_trans = right_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
    right_perturbed_action_quat_wxyz = torch3d_tf.matrix_to_quaternion(right_perturbed_action_gripper_4x4[:, :3, :3])
    right_perturbed_action_quat_xyzw = (
        torch.cat([right_perturbed_action_quat_wxyz[:, 1:], right_perturbed_action_quat_wxyz[:, 0].unsqueeze(1)], dim=1)
        .cpu()
        .numpy()
    )

    left_perturbed_action_trans = left_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
    left_perturbed_action_quat_wxyz = torch3d_tf.matrix_to_quaternion(left_perturbed_action_gripper_4x4[:, :3, :3])
    left_perturbed_action_quat_xyzw = (
        torch.cat([left_perturbed_action_quat_wxyz[:, 1:], left_perturbed_action_quat_wxyz[:, 0].unsqueeze(1)], dim=1)
        .cpu()
        .numpy()
    )

    # Compute the transformed indices for both right and left grippers
    right_trans_indices, right_rot_grip_indices = [], []
    left_trans_indices, left_rot_grip_indices = [], []
    for b in range(bs):
        bounds_np = bounds[b if layer > 0 else 0].cpu().numpy()

        # Right gripper
        right_trans_idx = utils.point_to_voxel_index(right_perturbed_action_trans[b], voxel_size, bounds_np)
        right_trans_indices.append(right_trans_idx.tolist())
        
        right_quat = utils.normalize_quaternion(right_perturbed_action_quat_xyzw[b])
        if right_quat[-1] < 0:
            right_quat = -right_quat
        right_disc_rot = utils.quaternion_to_discrete_euler(right_quat, rot_resolution)
        right_rot_grip_indices.append(right_disc_rot.tolist() + [int(right_action_rot_grip[b, 3].cpu().numpy())])

        # Left gripper
        left_trans_idx = utils.point_to_voxel_index(left_perturbed_action_trans[b], voxel_size, bounds_np)
        left_trans_indices.append(left_trans_idx.tolist())
        
        left_quat = utils.normalize_quaternion(left_perturbed_action_quat_xyzw[b])
        if left_quat[-1] < 0:
            left_quat = -left_quat
        left_disc_rot = utils.quaternion_to_discrete_euler(left_quat, rot_resolution)
        left_rot_grip_indices.append(left_disc_rot.tolist() + [int(left_action_rot_grip[b, 3].cpu().numpy())])

    # Convert to tensors for further processing
    right_perturbed_trans = torch.from_numpy(np.array(right_trans_indices)).to(device=device)
    right_perturbed_rot_grip = torch.from_numpy(np.array(right_rot_grip_indices)).to(device=device)

    left_perturbed_trans = torch.from_numpy(np.array(left_trans_indices)).to(device=device)
    left_perturbed_rot_grip = torch.from_numpy(np.array(left_rot_grip_indices)).to(device=device)

    # Update right and left action translation and rotation
    right_action_trans = right_perturbed_trans
    right_action_rot_grip = right_perturbed_rot_grip
    ...
    # Apply the same perturbation to the point clouds
    pcd = bimanual_perturb_se3(pcd, trans_shift_4x4, rot_shift_4x4, right_action_gripper_4x4, left_action_gripper_4x4, bounds)
