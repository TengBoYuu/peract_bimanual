...
def bimanual_apply_se3_augmentation1(
    pcd,
...
def bimanual_apply_se3_augmentation(
    ...
    # 4x4 matrix of right and left gripper poses
    right_action_gripper_4x4, left_action_gripper_4x4 = setup_gripper_matrices(
        right_action_gripper_pose, left_action_gripper_pose, identity_4x4
    )

    # Compute the center point
    center_4x4 = (right_action_gripper_4x4 + left_action_gripper_4x4) / 2

    perturb_attempts = 0
    while torch.any(right_action_trans < 0) or torch.any(left_action_trans < 0):
        perturb_attempts += 1
        if perturb_attempts > 100:
            raise Exception("Failing to perturb action and keep it within bounds.")
        
        # Translation perturbation
        trans_shift, trans_shift_4x4 = sample_translation_shift(
            bs, bounds, trans_aug_range, identity_4x4, device
        )
        
        # Rotation perturbation
        rot_shift_4x4 = sample_rotation_shift(rot_aug_range, rot_aug_resolution, bs, identity_4x4)

        # Center transformations
        right_centered = right_action_gripper_4x4 - center_4x4
        left_centered = left_action_gripper_4x4 - center_4x4

        # Apply rotation and translation
        right_perturbed_action_gripper_4x4 = torch.bmm(right_centered, rot_shift_4x4) + center_4x4
        right_perturbed_action_gripper_4x4[:, 0:3, 3] += trans_shift

        left_perturbed_action_gripper_4x4 = torch.bmm(left_centered, rot_shift_4x4) + center_4x4
        left_perturbed_action_gripper_4x4[:, 0:3, 3] += trans_shift

        # Update translations
        right_action_trans = torch.clamp(right_perturbed_action_gripper_4x4[:, 0:3, 3], min=bounds[:, :3], max=bounds[:, 3:])
        left_action_trans = torch.clamp(left_perturbed_action_gripper_4x4[:, 0:3, 3], min=bounds[:, :3], max=bounds[:, 3:])

        # Update rotations for right and left rot_grip
        right_action_rot_grip = convert_to_rotation_vector(right_perturbed_action_gripper_4x4[:, :3, :3], rot_resolution, device)
        left_action_rot_grip = convert_to_rotation_vector(left_perturbed_action_gripper_4x4[:, :3, :3], rot_resolution, device)
...
def setup_gripper_matrices(right_pose, left_pose, identity_4x4):
    """Set up right and left gripper matrices based on translation and quaternion rotation."""
    right_trans = right_pose[:, :3]
    right_quat_wxyz = torch.cat((right_pose[:, 6].unsqueeze(1), right_pose[:, 3:6]), dim=1)
    right_rot = torch3d_tf.quaternion_to_matrix(right_quat_wxyz)
    right_4x4 = identity_4x4.clone()
    right_4x4[:, :3, :3] = right_rot
    right_4x4[:, 0:3, 3] = right_trans

    left_trans = left_pose[:, :3]
    left_quat_wxyz = torch.cat((left_pose[:, 6].unsqueeze(1), left_pose[:, 3:6]), dim=1)
    left_rot = torch3d_tf.quaternion_to_matrix(left_quat_wxyz)
    left_4x4 = identity_4x4.clone()
    left_4x4[:, :3, :3] = left_rot
    left_4x4[:, 0:3, 3] = left_trans

    return right_4x4, left_4x4

def sample_translation_shift(bs, bounds, trans_aug_range, identity_4x4, device):
    trans_range = (bounds[:, 3:] - bounds[:, :3]) * trans_aug_range.to(device=device)
    trans_shift = trans_range * utils.rand_dist((bs, 3)).to(device=device)
    trans_shift_4x4 = identity_4x4.clone()
    trans_shift_4x4[:, 0:3, 3] = trans_shift
    return trans_shift, trans_shift_4x4

def sample_rotation_shift(rot_aug_range, rot_aug_resolution, bs, identity_4x4):
    roll = utils.rand_discrete((bs, 1), min=-rot_aug_range[0], max=rot_aug_range[0]) * np.deg2rad(rot_aug_resolution)
    pitch = utils.rand_discrete((bs, 1), min=-rot_aug_range[1], max=rot_aug_range[1]) * np.deg2rad(rot_aug_resolution)
    yaw = utils.rand_discrete((bs, 1), min=-rot_aug_range[2], max=rot_aug_range[2]) * np.deg2rad(rot_aug_resolution)
    rot_shift_3x3 = torch3d_tf.euler_angles_to_matrix(torch.cat((roll, pitch, yaw), dim=1), "XYZ")
    rot_shift_4x4 = identity_4x4.clone()
    rot_shift_4x4[:, :3, :3] = rot_shift_3x3
    return rot_shift_4x4

def convert_to_rotation_vector(rotation_matrix, rot_resolution, device):
    """Convert rotation matrix to discrete rotation vector."""
    quat = torch3d_tf.matrix_to_quaternion(rotation_matrix)
    euler_rot = torch3d_tf.quaternion_to_euler_angles(quat, "XYZ")
    rot_grip = torch.round(euler_rot / rot_resolution).to(device=device)
    return rot_grip
