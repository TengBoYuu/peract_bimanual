...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            """
            # Assuming matrix is a Bx4x4 tensor
            B = matrix.shape[0]
            ...
            translated_matrix = matrix.clone()
            translated_matrix[:, :3, 3] += translation_to_origin.squeeze(2)

            # Rotate the matrix
            rotated_matrix = torch.matmul(rotation_matrix, translated_matrix[:, :3, :])
            rotated_matrix = torch.cat([rotated_matrix, translated_matrix[:, 3:, :]], dim=1)

            # Translate the matrix back
            final_matrix = rotated_matrix.clone()
            final_matrix[:, :3, 3] += translation.squeeze(2)
