...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            transform_matrices = torch.zeros(B, 4, 4, device=matrix.device)

            # Fill in the rotation part
            transform_matrices[:, :3, :3] = rotation_matrix

            # Fill in the translation part
            transform_matrices[:, :3, 3] = translation.squeeze(1) + (C.squeeze(1) - C.squeeze(1) @ rotation_matrix)

            # Set the last row for homogeneous coordinates
            transform_matrices[:, 3, 3] = 1

            # Apply the transformation
            # To transform the original matrix, multiply the transformation matrix by the original matrix
            transformed_matrix = torch.bmm(transform_matrices, matrix)
