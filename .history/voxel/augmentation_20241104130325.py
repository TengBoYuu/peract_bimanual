...
if __name__ == "__main__":
    ...
    def convert_to_open3d(pcd_tensor):
        # Convert tensor to numpy and reshape for Open3D
        pcd_np = pcd_tensor.squeeze().cpu().numpy()
        pcd_np = pcd_np.reshape(-1, 3)
        pcd_open3d = o3d.geometry.PointCloud()
        pcd_open3d.points = o3d.utility.Vector3dVector(pcd_np)
        return pcd_open3d

    def visualize_point_clouds(before_pcd, after_pcd, filename=None, hold_time=5):
        before_o3d = convert_to_open3d(before_pcd)
        after_o3d = convert_to_open3d(after_pcd)

        before_o3d.paint_uniform_color([1, 0, 0])  # Red for before
        after_o3d.paint_uniform_color([0, 1, 0])   # Green for after

        # o3d.visualization.draw_geometries([before_o3d, after_o3d], window_name="Before and After")
        vis = o3d.visualization.Visualizer()
        vis.create_window(width=640, height=480)

        combined_pcd = [before_o3d, after_o3d]
        for pcd in combined_pcd:
            vis.add_geometry(pcd)
            vis.update_geometry(pcd)

        # Get the view control object and set the camera parameters to look along the Z-axis
        ctr = vis.get_view_control()
        parameters = o3d.io.read_pinhole_camera_parameters("ScreenCamera_2024-10-21-16-45-48.json")
        ...
        print(f"[visualize_ply] Combined point cloud image saved to '{filename}'.")
