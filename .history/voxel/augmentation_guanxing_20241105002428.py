...
def bimanual_apply_se3_augmentation(
    ...
    # matrix[:, 0:3, 3]: [bs, 3]
    # rotation_matrix: [bs, 3, 3]
    translation_vec = matrix[:, 0:3, 3].unsqueeze(-1)  # [bs, 3, 1]
    rotated_translation = torch.bmm(rotation_matrix, translation_vec)  # [bs, 3, 1]
    rotated_translation = rotated_translation.squeeze(-1)  # [bs, 3]
    matrix[:, 0:3, 3] = rotated_translation  # [bs, 3]

    # Add C back to the translation part
    matrix[:, 0:3, 3] += C  # [bs, 3]

    # Add translation
    matrix[:, 0:3, 3] += translation  # [bs, 3]

    # Rotate the rotation part of the matrix
    # matrix[:, :3, :3]: [bs, 3, 3]
    # rotation_matrix: [bs, 3, 3]
    matrix[:, :3, :3] = torch.matmul(matrix[:, :3, :3], rotation_matrix)  # [bs, 3, 3]
            return matrix
