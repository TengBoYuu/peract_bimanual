...
def bimanual_apply_se3_augmentation(
    ...
    # 计算中心点
    center_trans_3x1 = ((right_action_gripper_4x4[:, :3, 3] + left_action_gripper_4x4[:, :3, 3]) / 2).unsqueeze(-1)
    
    # 将 right 和 left 平移到中心点，以中心点为旋转基准
    right_centered_trans = right_action_trans - center_trans_3x1.squeeze()
    left_centered_trans = left_action_trans - center_trans_3x1.squeeze()
    ...
    # 对右臂、左臂和 pcd 分别应用平移和旋转
    right_perturbed_trans = torch.bmm(right_centered_trans.unsqueeze(-1).transpose(2, 1), rot_shift_4x4[:, :3, :3]).squeeze(-1)
    left_perturbed_trans = torch.bmm(left_centered_trans.unsqueeze(-1).transpose(2, 1), rot_shift_4x4[:, :3, :3]).squeeze(-1)
    
    # 添加扰动
    right_perturbed_trans += trans_shift.squeeze()
    left_perturbed_trans += trans_shift.squeeze()
    
    # 恢复至原中心位置
    right_perturbed_trans += center_trans_3x1.squeeze()
    left_perturbed_trans += center_trans_3x1.squeeze()
    
    # 应用到点云
    pcd = bimanual_perturb_se3(pcd, trans_shift_4x4, rot_shift_4x4, right_action_gripper_4x4, left_action_gripper_4x4, bounds)

    return right_perturbed_trans, left_perturbed_trans, pcd
