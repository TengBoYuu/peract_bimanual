...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            """
            matrix[0:3, 3] -= C
            matrix[0:3, 3] = torch.matmul(rotation_matrix, matrix[0:3, 3])
            matrix[0:3, 3] += C
        ...
        left_perturbed_action_gripper_4x4 = transform(
            left_perturbed_action_gripper_4x4.detach().clone().squeeze(),
...
if __name__ == "__main__":
    ...
    def plot_point_cloud(pcd, left_pose, right_pose, title, filename):
        ...
        ax.set_zlabel('Z')
        # legend
        ax.scatter(0, 0, 0, c='b', marker='o', s=1, label='Point Cloud')
        ax.scatter(0, 0, 0, c='r', marker='o', s=40, label='Left Gripper Pose')
        ax.scatter(0, 0, 0, c='g', marker='o', s=40, label='Right Gripper Pose')
        ax.legend()
