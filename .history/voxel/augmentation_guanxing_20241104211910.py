...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            """
            # Create a batch of 4x4 identity matrices
            batch_size = matrix.shape[0]
            identity_matrix = torch.eye(4).unsqueeze(0).repeat(batch_size, 1, 1).to(matrix.device)

            # Fill the rotation part of the homogeneous matrix
            identity_matrix[:, :3, :3] = rotation_matrix

            # Fill the translation part of the homogeneous matrix
            identity_matrix[:, :3, 3] = translation

            # Translate the center point C to the origin
            matrix_translated = matrix.clone()
            matrix_translated[:, :3, 3] -= C

            # Apply the homogeneous transformation
            transformed_matrix = torch.bmm(matrix_translated, identity_matrix)

            # Translate the transformed matrix back to the original center
            transformed_matrix[:, :3, 3] += C
