...
def bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        # right_perturbed_action_gripper_4x4 = torch.bmm(
        #     average_action_gripper_4x4, rot_shift_4x4
        # )
        ...
        # Step 2: 使用平均矩阵进行旋转
        average_perturbed_action_gripper_4x4 = torch.bmm(average_action_gripper_4x4, rot_shift_4x4)
        average_perturbed_action_gripper_4x4[:, 0:3, 3] += trans_shift

        # 获取平均位置的平移和四元数（用于参考）
        average_perturbed_action_trans = (
            average_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
        )
        average_perturbed_action_quat_wxyz = torch3d_tf.matrix_to_quaternion(
            average_perturbed_action_gripper_4x4[:, :3, :3]
        )

        # Step 3: 在平均矩阵基础上分别应用右、左抓取器的旋转
        # 对右抓取器应用旋转
        right_perturbed_action_gripper_4x4 = torch.bmm(average_perturbed_action_gripper_4x4, right_action_gripper_4x4)
        right_perturbed_action_trans = (
            right_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
        )
        right_perturbed_action_quat_wxyz = torch3d_tf.matrix_to_quaternion(
        ...
        # 对左抓取器应用旋转
        left_perturbed_action_gripper_4x4 = torch.bmm(average_perturbed_action_gripper_4x4, left_action_gripper_4x4)
        left_perturbed_action_trans = (
            left_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
        )
        left_perturbed_action_quat_wxyz = torch3d_tf.matrix_to_quaternion(
