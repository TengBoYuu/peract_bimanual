...
def my_bimanual_apply_se3_augmentation(
    pcd,
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        # ) * np.deg2rad(rot_aug_resolution)
        # for debugging
        yaw = torch.tensor([[35.0]]).to(device=device)
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            Args:
                matrix: Bx4x4 matrix to be transformed. tensor.
                C: Bx3x1 Center of rotation. tensor.
                translation: Bx3x1 Translation vector. tensor.
                rotation_matrix: Bx3x3 rotation matrix. tensor.
            """
             # Extract the batch size
            B = matrix.shape[0]

            # Create a tensor for the translation part
            translation_matrix = torch.zeros(B, 4, 4, device=matrix.device)
            translation_matrix[:, 3, 3] = 1  # Homogeneous coordinate

            # Fill in the translation matrix
            translation_matrix[:, :3, 3] = translation.squeeze(1)  # Add translation vector
            
            # Create the rotation matrix in a 4x4 form
            rotation_homogeneous = torch.zeros(B, 4, 4, device=matrix.device)
            rotation_homogeneous[:, :3, :3] = rotation_matrix
            rotation_homogeneous[:, 3, 3] = 1  # Homogeneous coordinate

            # Step 1: Translate to the center C
            translation_to_C = torch.zeros(B, 4, 4, device=matrix.device)
            translation_to_C[:, 3, 3] = 1
            translation_to_C[:, :3, 3] = -C.squeeze(1)  # Move to the center
            
            # Step 2: Apply rotation
            # The combined transformation: Translate to center -> Rotate -> Translate back
            transformation_matrix = translation_matrix @ rotation_homogeneous @ translation_to_C

            # Apply transformation to the original matrix
            transformed_matrix = transformation_matrix @ matrix
        ...
        # left_perturbed_action_gripper_4x4 = left_action_gripper_4x4.detach().clone()
        # left_perturbed_action_gripper_4x4[:, 0:3, 3] -= center_action
        # left_perturbed_action_gripper_4x4 = torch.bmm(
        #     left_perturbed_action_gripper_4x4, rot_shift_4x4
        # )
        # left_perturbed_action_gripper_4x4[:, 0:3, 3] += center_action
        # left_perturbed_action_gripper_4x4[:, 0:3, 3] += trans_shift
...
if __name__ == "__main__":
    ...
    outputs = my_bimanual_apply_se3_augmentation(
        pcd,
        right_action_gripper_pose,
        right_action_trans,
        right_action_rot_grip,
