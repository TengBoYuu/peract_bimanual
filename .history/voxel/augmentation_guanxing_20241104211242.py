    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            Args:
                ...
                C: Bx3 Center of rotation. tensor.
                translation: Bx3 Translation vector. tensor.
                rotation_matrix: Bx3x3 rotation matrix. tensor.
            """
            
            # Create homogeneous transformation matrices
            homogeneous_rotation_matrix = torch.cat([rotation_matrix, torch.zeros_like(rotation_matrix[:, :1, :])], dim=2)
            homogeneous_rotation_matrix = torch.cat([homogeneous_rotation_matrix, torch.tensor([[[0, 0, 0, 1]]]).repeat(matrix.shape[0], 1, 1)], dim=1)

            # Translate the center to the origin
            translation_to_origin = -C
            translation_matrix_to_origin = torch.eye(4).repeat(matrix.shape[0], 1, 1)
            translation_matrix_to_origin[:, :3, 3] = translation_to_origin

            # Translate back from the origin
            translation_from_origin = C + translation
            translation_matrix_from_origin = torch.eye(4).repeat(matrix.shape[0], 1, 1)
            translation_matrix_from_origin[:, :3, 3] = translation_from_origin

            # Apply transformations
            transformed_matrix = torch.matmul(translation_matrix_from_origin, torch.matmul(homogeneous_rotation_matrix, torch.matmul(translation_matrix_to_origin, matrix)))
