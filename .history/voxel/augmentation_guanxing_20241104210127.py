...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            ...
            """
            # 将C和translation从Bx3x1转换为Bx3以进行矩阵乘法
            C = C.view(-1, 3)
            translation = translation.view(-1, 3)
            
            # 计算旋转和平移后的矩阵
            rotated_matrix = torch.matmul(matrix[:, :3, :3], rotation_matrix) + \
                            (matrix[:, :3, 3] - C) * rotation_matrix + C + translation.unsqueeze(2)
            
            # 将变换后的矩阵放回原始4x4矩阵中
            transformed_matrix = torch.cat((rotated_matrix, matrix[:, :3, 3:4]), dim=2)
