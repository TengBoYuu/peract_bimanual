...
def apply_transformation_to_action(
    ...
    # 对平移部分的处理（将 right_action_trans 和 left_action_trans 视为点进行变换）
    right_action_trans_centered = right_action_trans - center_4x4[:, :3, 3]
    left_action_trans_centered = left_action_trans - center_4x4[:, :3, 3]

    # 旋转操作
    right_action_trans_rotated = torch.bmm(
        rot_shift_4x4[:, :3, :3], right_action_trans_centered.unsqueeze(-1)
    ).squeeze(-1)

    left_action_trans_rotated = torch.bmm(
        rot_shift_4x4[:, :3, :3], left_action_trans_centered.unsqueeze(-1)
    ).squeeze(-1)

    # 平移回中心
    right_action_trans = right_action_trans_rotated + center_4x4[:, :3, 3]
    left_action_trans = left_action_trans_rotated + center_4x4[:, :3, 3]
