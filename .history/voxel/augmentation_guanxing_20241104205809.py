...
def my_bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        def transform(matrix, C, translation, rotation_matrix):
            """
            Rotate and translate a 4x4 matrix around the center C.
            ...
            translation_to_origin = torch.tensor([[1, 0, 0, -C[0]],
                                                [0, 1, 0, -C[1]],
                                                [0, 0, 1, -C[2]],
                                                [0, 0, 0, 1]], dtype=torch.float)
            matrix_translated_to_origin = torch.matmul(translation_to_origin, matrix)

            # Apply rotation
            matrix_rotated = torch.zeros((4, 4), dtype=torch.float)
            matrix_rotated[:3, :3] = rotation_matrix
            matrix_rotated[3, 3] = 1
            result_after_rotation = torch.matmul(matrix_rotated, matrix_translated_to_origin)

            # Translate back to the original position
            translation_back = torch.tensor([[1, 0, 0, C[0]],
                                            [0, 1, 0, C[1]],
                                            [0, 0, 1, C[2]],
                                            [0, 0, 0, 1]], dtype=torch.float)
            final_result = torch.matmul(translation_back, result_after_rotation)

            # Apply translation
            translation_matrix = torch.tensor([[1, 0, 0, translation[0]],
                                            [0, 1, 0, translation[1]],
                                            [0, 0, 1, translation[2]],
                                            [0, 0, 0, 1]], dtype=torch.float)

            return torch.matmul(translation_matrix, final_result)
