...
def bimanual_apply_se3_augmentation(
    ...
    while torch.any(right_perturbed_trans < 0) or torch.any(left_perturbed_trans < 0):
        ...
        center_trans_3x1 = (
            (right_action_gripper_4x4[:, :3, 3] + left_action_gripper_4x4[:, :3, 3])
            / 2
        ).unsqueeze(-1)  # [bs, 3, 1]
        ...
        right_action_trans_rotated = torch.bmm(
            rot_shift_4x4[:, :3, :3], right_action_trans_centered
        )  # [bs, 3, 1]
        right_action_trans_transformed = (
            right_action_trans_rotated.squeeze(-1) + center_trans_3x1.squeeze(-1) + trans_shift
        )  # [bs, 3]
        ...
        left_action_trans_rotated = torch.bmm(
            rot_shift_4x4[:, :3, :3], left_action_trans_centered
        )  # [bs, 3, 1]
        left_action_trans_transformed = (
            left_action_trans_rotated.squeeze(-1) + center_trans_3x1.squeeze(-1) + trans_shift
        )  # [bs, 3]
        ...
        right_action_rot_transformed = torch.bmm(
            right_action_rot_mat, rot_shift_4x4[:, :3, :3]
        )  # [bs, 3, 3]
        left_action_rot_transformed = torch.bmm(
            left_action_rot_mat, rot_shift_4x4[:, :3, :3]
        )  # [bs, 3, 3]
        ...
        right_action_quat_xyzw = torch.cat(
            [
                right_action_quat_wxyz[:, 1:],
                right_action_quat_wxyz[:, 0].unsqueeze(1),
            ],
            dim=1,
        )
        left_action_quat_xyzw = torch.cat(
            [
                left_action_quat_wxyz[:, 1:],
                left_action_quat_wxyz[:, 0].unsqueeze(1),
            ],
            dim=1,
        )
        ...
        for b in range(bs):
            ...
            # Right arm
            right_trans_idx = utils.point_to_voxel_index(
                right_action_trans_transformed[b], voxel_size, bounds_np
            )
            right_trans_indices.append(right_trans_idx.tolist())

            right_quat = right_action_quat_xyzw[b].cpu().numpy()
            ...
            right_rot_grip_indices.append(
                right_disc_rot.tolist()
                + [int(right_action_rot_grip[b, -1].cpu().numpy())]
            )

            # Left arm
            left_trans_idx = utils.point_to_voxel_index(
                left_action_trans_transformed[b], voxel_size, bounds_np
            )
            left_trans_indices.append(left_trans_idx.tolist())

            left_quat = left_action_quat_xyzw[b].cpu().numpy()
            ...
            left_rot_grip_indices.append(
                left_disc_rot.tolist()
                + [int(left_action_rot_grip[b, -1].cpu().numpy())]
            )

        # Convert lists to tensors
        right_perturbed_trans = torch.from_numpy(np.array(right_trans_indices)).to(
            device=device
        )
        right_perturbed_rot_grip = torch.from_numpy(np.array(right_rot_grip_indices)).to(
            device=device
        )
        left_perturbed_trans = torch.from_numpy(np.array(left_trans_indices)).to(
            device=device
        )
        left_perturbed_rot_grip = torch.from_numpy(np.array(left_rot_grip_indices)).to(
            device=device
        )
