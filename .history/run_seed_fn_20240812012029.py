...
def run_seed(
    ...
    if cfg.method.name == "ARM":
        ...
        # 加载所有的 Replay 文件
        replay_files = [os.path.join(replay_path, f) for f in os.listdir(replay_path) if f.endswith('.replay')]
        replay_data_dict = {}  # 初始化一个字典用于存储多个 replay_data
        counter = 1  # 初始化计数器

        for replay_file in replay_files:
            with open(replay_file, 'rb') as f:
                # 动态加载数据并存储在字典中
                replay_data_dict[f'replay_data_{counter}'] = pickle.load(f)
            
            print(f"Loaded {replay_file} into replay_data_{counter}")
            
            # 在原文件名后添加递增的数字
            file_name, file_ext = os.path.splitext(os.path.basename(replay_file))
            output_file_path_before = os.path.join(output_path, f"{file_name}_{counter}{file_ext}")
            
            # 保存带有新名称的文件
            with open(output_file_path_before, 'wb') as out_f:
                pickle.dump(replay_data_dict[f'replay_data_{counter}'], out_f)
            
            # 调用 _add 方法
            replay_buffer._add(replay_data_dict[f'replay_data_{counter}'])
