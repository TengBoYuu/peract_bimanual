...
class OptionSelector(nn.Module):  # 继承自 torch.nn.Module
    ...
    def forward(self, rgb_list, lang_input):
        ...
        for i in range(predicted_class.size(0)):  # 遍历整个 batch 中每个样本的类别
            class_idx = predicted_class[i].item()  # 获取每个样本的最高类别索引

            # 根据类别找到对应的 vocabulary key
